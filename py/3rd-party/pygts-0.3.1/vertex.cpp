...
static PyObject* is_ok(PygtsVertex* self, PyObject* args)
{
...
static PyObject* is_unattached(PygtsVertex* self, PyObject* args)
{
	guint n;
...
static PyObject* is_boundary(PygtsVertex* self, PyObject* args)
{
	PyObject*    s_;
	PygtsObject* s;
	...
	if (gts_vertex_is_boundary(PYGTS_VERTEX_AS_GTS_VERTEX(self), PYGTS_SURFACE_AS_GTS_SURFACE(s))) {
		Py_INCREF(Py_True);
		return Py_True;
	} else {
		Py_INCREF(Py_False);
...
static PyObject* is_connected(PygtsVertex* self, PyObject* args)
{
	PyObject*    v_;
	PygtsVertex* v;
	...
	if (gts_vertices_are_connected(PYGTS_VERTEX_AS_GTS_VERTEX(self), PYGTS_VERTEX_AS_GTS_VERTEX(v)) != NULL) {
		Py_INCREF(Py_True);
		return Py_True;
	} else {
		Py_INCREF(Py_False);
...
static PyObject* replace(PygtsVertex* self, PyObject* args)
{
	PyObject*    p2_;
	PygtsVertex* p2;
	...
	if (self != p2) {
		...
		gts_vertex_replace(PYGTS_VERTEX_AS_GTS_VERTEX(self), PYGTS_VERTEX_AS_GTS_VERTEX(p2));
...
static PyObject* encroaches(PygtsVertex* self, PyObject* args)
{
	PyObject*  e_;
	PygtsEdge* e;
	...
	if (gts_vertex_encroaches_edge(PYGTS_VERTEX_AS_GTS_VERTEX(self), PYGTS_EDGE_AS_GTS_EDGE(e))) {
		Py_INCREF(Py_True);
		return Py_True;
	} else {
		Py_INCREF(Py_False);
...
static PyObject* triangles(PygtsVertex* self, PyObject* args)
{
	GSList *       triangles, *t;
...
static int init(PygtsVertex* self, PyObject* args, PyObject* kwds)
{
	gint ret;
...
PyTypeObject PygtsVertexType = {
	PyVarObject_HEAD_INIT(NULL, 0) "gts.Vertex", /* tp_name */
...
gboolean pygts_vertex_is_ok(PygtsVertex* v)
{
	...
	parent = g_slist_find(GTS_VERTEX(obj->gtsobj)->segments, obj->gtsobj_parent);
...
static GtsObject* parent(GtsVertex* v1)
{
	...
	p1 = GTS_POINT(v1);
	if ((v2 = gts_vertex_new(pygts_parent_vertex_class(), p1->x, p1->y, p1->z + 1)) == NULL) {
		PyErr_SetString(PyExc_MemoryError, "could not create parent");
		return NULL;
	}

	/* Create and return the parent */
	if ((p = gts_segment_new(pygts_parent_segment_class(), v1, v2)) == NULL) {
		PyErr_SetString(PyExc_MemoryError, "could not create parent");
...
PygtsVertex* pygts_vertex_new(GtsVertex* v)
{
	PyObject *   args, *kwds;
	PygtsObject* vertex;

	/* Check for Vertex in the object table */
	if ((vertex = PYGTS_OBJECT(g_hash_table_lookup(obj_table, GTS_OBJECT(v)))) != NULL) {
		Py_INCREF(vertex);
...
GtsSegmentClass* pygts_parent_segment_class(void)
{
	...
	if (klass == NULL) {
		...
		GtsObjectClassInfo pygts_parent_segment_info = { "PygtsParentSegment",
...
GtsVertexClass* pygts_parent_vertex_class(void)
{
	...
	if (klass == NULL) {
		...
		GtsObjectClassInfo pygts_parent_vertex_info = { "PygtsParentVertex",
