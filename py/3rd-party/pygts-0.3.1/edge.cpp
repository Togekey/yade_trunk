...
static PyObject* is_ok(PygtsEdge* self, PyObject* args)
{
...
static PyObject* is_unattached(PygtsEdge* self, PyObject* args)
{
	guint n;
...
static PyObject* face_number(PygtsEdge* self, PyObject* args)
{
	PyObject*   s_;
	GtsSurface* s;
	...
	return Py_BuildValue("i", gts_edge_face_number(PYGTS_EDGE_AS_GTS_EDGE(self), s));
}


static PyObject* belongs_to_tetrahedron(PygtsEdge* self, PyObject* args)
{
	SELF_CHECK
...
static PyObject* is_boundary(PygtsEdge* self, PyObject* args)
{
	PyObject*   s_;
	GtsSurface* s;
...
static PyObject* contacts(PygtsEdge* self, PyObject* args)
{
	SELF_CHECK

	return Py_BuildValue("i", gts_edge_is_contact(PYGTS_EDGE_AS_GTS_EDGE(self)));
}
...
*/
	...
	{ "belongs_to_tetrahedron",
	  (PyCFunction)belongs_to_tetrahedron,
	  METH_NOARGS,
...
static PyObject* new_(PyTypeObject* type, PyObject* args, PyObject* kwds)
{
	...
	if (alloc_gtsobj) {
		...
		/* Create the GtsEdge */
		edge = GTS_OBJECT(gts_edge_new(gts_edge_class(), GTS_VERTEX(v1->gtsobj), GTS_VERTEX(v2->gtsobj)));
		if (edge == NULL) {
...
static int init(PygtsEdge* self, PyObject* args, PyObject* kwds)
{
	gint ret;
...
PyTypeObject PygtsEdgeType = {
	PyVarObject_HEAD_INIT(NULL, 0) "gts.Edge", /* tp_name */
...
gboolean pygts_edge_is_ok(PygtsEdge* e)
{
	...
	parent = g_slist_find(GTS_EDGE(obj->gtsobj)->triangles, obj->gtsobj_parent);
...
static GtsObject* parent(GtsEdge* e1)
{
	...
	p1 = GTS_POINT(v1);
	p2 = GTS_POINT(v2);
	v3 = gts_vertex_new(pygts_parent_vertex_class(), p1->x + p2->x, p1->y + p2->y, p1->z + p2->z);
	if (v3 == NULL) {
	...
	if ((p = gts_triangle_new(pygts_parent_triangle_class(), e1, e2, e3)) == NULL) {
		gts_object_destroy(GTS_OBJECT(e2));
...
PygtsEdge* pygts_edge_new(GtsEdge* e)
{
	...
	if ((edge = PYGTS_OBJECT(g_hash_table_lookup(obj_table, GTS_OBJECT(e)))) != NULL) {
		Py_INCREF(edge);
...
GtsTriangleClass* pygts_parent_triangle_class(void)
{
	...
	if (klass == NULL) {
		...
		GtsObjectClassInfo pygts_parent_triangle_info = { "PygtsParentTriangle",
...
GtsEdgeClass* pygts_parent_edge_class(void)
{
	...
	if (klass == NULL) {
		...
		GtsObjectClassInfo pygts_parent_edge_info = { "PygtsParentEdge",
