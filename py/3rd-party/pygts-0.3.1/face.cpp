...
static PyObject* is_ok(PygtsFace* self, PyObject* args)
{
...
static PyObject* is_unattached(PygtsFace* self, PyObject* args)
{
	guint n;
...
static PyObject* neighbor_number(PygtsFace* self, PyObject* args)
{
	...
	return Py_BuildValue("i", gts_face_neighbor_number(PYGTS_FACE_AS_GTS_FACE(self), PYGTS_SURFACE_AS_GTS_SURFACE(s)));
}
...
static PyObject* neighbors(PygtsFace* self, PyObject* args)
{
	...
	N = gts_face_neighbor_number(PYGTS_FACE_AS_GTS_FACE(self), PYGTS_SURFACE_AS_GTS_SURFACE(s));
...
static PyObject* is_compatible(PygtsFace* self, PyObject* args)
{
	...
	if (t != NULL) {
		if ((e = gts_triangles_common_edge(PYGTS_TRIANGLE_AS_GTS_TRIANGLE(self), PYGTS_TRIANGLE_AS_GTS_TRIANGLE(t))) == NULL) {
			PyErr_SetString(PyExc_RuntimeError, "Faces do not share common edge");
			return NULL;
		}
		if (gts_triangles_are_compatible(PYGTS_TRIANGLE_AS_GTS_TRIANGLE(self), PYGTS_TRIANGLE_AS_GTS_TRIANGLE(t), e) == TRUE) {
			Py_INCREF(Py_True);
			return Py_True;
		}
	} else {
		if (gts_face_is_compatible(PYGTS_FACE_AS_GTS_FACE(self), PYGTS_SURFACE_AS_GTS_SURFACE(s)) == TRUE) {
			Py_INCREF(Py_True);
			return Py_True;
		}
	}
...
static PyObject* is_on(PygtsFace* self, PyObject* args)
{
	...
	if (gts_face_has_parent_surface(PYGTS_FACE_AS_GTS_FACE(self), PYGTS_SURFACE_AS_GTS_SURFACE(s))) {
		Py_INCREF(Py_True);
		return Py_True;
	} else {
		Py_INCREF(Py_False);
...
static PyObject* new_(PyTypeObject* type, PyObject* args, PyObject* kwds)
{
	...
	if (alloc_gtsobj) {
		...
		if (!((e1 != NULL && e2 != NULL && e3 != NULL) || (v1 != NULL && v2 != NULL && v3 != NULL))) {
			PyErr_SetString(PyExc_TypeError, "three Edge or three Vertex objects expected");
			return NULL;
		}
...
static int init(PygtsFace* self, PyObject* args, PyObject* kwds)
{
	gint ret;
...
PyTypeObject PygtsFaceType = {
	PyVarObject_HEAD_INIT(NULL, 0) "gts.Face", /* tp_name */
...
gboolean pygts_face_is_ok(PygtsFace* f)
{
	...
	parent = g_slist_find(GTS_FACE(obj->gtsobj)->surfaces, obj->gtsobj_parent);
...
static GtsObject* parent(GtsFace* face)
{
	...
	p = gts_surface_new(gts_surface_class(), gts_face_class(), gts_edge_class(), gts_vertex_class());
...
PygtsFace* pygts_face_new(GtsFace* f)
{
	...
	if ((face = PYGTS_OBJECT(g_hash_table_lookup(obj_table, GTS_OBJECT(f)))) != NULL) {
		Py_INCREF(face);
