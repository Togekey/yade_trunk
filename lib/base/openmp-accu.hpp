...
namespace serialization {
	...
	template <class Archive> void save(Archive& ar, const yade::OpenMPAccumulator<Real>& a, unsigned int /*version*/)
	{
		Real value = a.get();
		ar&  BOOST_SERIALIZATION_NVP(value);
	}
	template <class Archive> void load(Archive& ar, yade::OpenMPAccumulator<Real>& a, unsigned int /*version*/)
	{
		Real value;
		ar&  BOOST_SERIALIZATION_NVP(value);
		a.set(value);
	}
	template <class Archive> void save(Archive& ar, const yade::OpenMPArrayAccumulator<Real>& a, unsigned int /*version*/)
	{
		size_t size = a.size();
	...
	template <class Archive> void load(Archive& ar, yade::OpenMPArrayAccumulator<Real>& a, unsigned int /*version*/)
	{
		size_t size;
		ar&    BOOST_SERIALIZATION_NVP(size);
		a.resize(size);
