...
struct DynLibDispatcher_Item2D {
	...
	DynLibDispatcher_Item2D(int a, int b, std::string c)
	        : ix1(a)
	        , ix2(b)
	        , functorName(c) {};
};
struct DynLibDispatcher_Item1D {
	int         ix1;
	std::string functorName;
	DynLibDispatcher_Item1D(int a, std::string c)
	        : ix1(a)
	        , functorName(c) {};
};
///
...
private:
	...
	template <class Head, class Tail> struct Matrix<Loki::Typelist<Head, Tail>> {
...
public:
	...
	{
		if (arg1->getClassIndex() < 0 || arg2->getClassIndex() < 0)
			throw runtime_error(
			        "No functor for types " + arg1->getClassName() + " (index " + boost::lexical_cast<string>(arg1->getClassIndex()) + ") + "
...
#define _DISP_TRACE(msg)
		...
		int foundIx1, foundIx2;
		int maxDp1 = -1, maxDp2 = -1;
...
private:
	template <bool useSymmetry, class BaseClassTrait1, class BaseClassTrait2, typename... Args> struct InvocationTraits {
	...
	template <class BaseClassTrait, typename... Args> struct InvocationTraits<true, BaseClassTrait, BaseClassTrait, Args...> {
...
public:
	template <typename... Args> ResultType operator()(shared_ptr<BaseClass1>& base1, shared_ptr<BaseClass2>& base2, Args... args)
	{
		int index1, index2;
