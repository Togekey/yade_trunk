...
template <> inline void glMultMatrix<LDOUBL>(const LDOUBL* m)
{
	double mm[16];
	for (int i = 0; i < 16; i++)
		mm[i] = (double)m[i];
...
template <> inline void glColor4<LDOUBL>(LDOUBL red, LDOUBL green, LDOUBL blue, LDOUBL alpha)
{
...
template <> inline void glMaterialv<Vector3r>(GLenum face, GLenum pname, const Vector3r params)
{
	const GLfloat _p[3] = { (float)params[0], (float)params[1], (float)params[2] };
