...
#define F77_FUNC(func) func##_
	...
	template <class _Tesselation, class FlowType> void FlowBoundingSphereLinSolv<_Tesselation, FlowType>::swapFwd(double* v, int i)
	{
		double temp = v[i];
		v[i]        = v[i + 1];
		v[i + 1]    = temp;
...
#pragma omp master
	...
	template <class _Tesselation, class FlowType> void FlowBoundingSphereLinSolv<_Tesselation, FlowType>::sortV(int k1, int k2, int* is, double* ds)
	{
		for (int k = k1; k < k2; k++) {
			int kk = k;
	...
	template <class _Tesselation, class FlowType> int FlowBoundingSphereLinSolv<_Tesselation, FlowType>::eigenSolve(Real dt)
	{
		...
		Eigen::VectorXd eb(ncols);
		Eigen::VectorXd ex(ncols);
		for (int k = 0; k < ncols; k++)
			eb[k] = T_bv[k];
