...
namespace CGT {
	...
	template <class Tesselation> double Network<Tesselation>::volumePoreVoronoiFraction(CellHandle& cell, int& j, bool reuseFacetData)
	{
		Point& p1 = cell->info();
	...
	template <class Tesselation> double Network<Tesselation>::volumeSolidPore(const CellHandle& cell)
	{
		double Vsolid = 0;
		for (int i = 0; i < 4; i++) {
	...
	double Network<Tesselation>::volumeSingleFictiousPore(
	...
	template <class Tesselation>
	double Network<Tesselation>::volumeDoubleFictiousPore(
	        const VertexHandle& SV1, const VertexHandle& SV2, const VertexHandle& SV3, const Point& PV1, const Point& PV2, CVector& facetSurface)
	{
		double    A[3], B[3];
		Boundary& bi1 = boundary(SV1->info().id());
	...
	template <class Tesselation> double Network<Tesselation>::sphericalTriangleArea(Sphere STA1, Sphere STA2, Sphere STA3, Point PTA1)
	{
	...
	template <class Tesselation> Real Network<Tesselation>::fastSolidAngle(const Point& STA1, const Point& PTA1, const Point& PTA2, const Point& PTA3)
	{
		...
		double detM = M[0][0] * (M[1][1] * M[2][2] - M[2][1] * M[1][2]) + M[1][0] * (M[2][1] * M[0][2] - M[0][1] * M[2][2])
	...
	template <class Tesselation> double Network<Tesselation>::surfaceSolidThroat(CellHandle cell, int j, bool slipBoundary, bool reuseFacetData)
	{
		if (!reuseFacetData)
			facetNFictious = detectFacetFictiousVertices(cell, j);
		Point& p1 = cell->info();
	...
	template <class Tesselation> double Network<Tesselation>::surfaceSolidThroatInPore(CellHandle cell, int j, bool slipBoundary, bool reuseFacetData)
	{
		if (!reuseFacetData)
			facetNFictious = detectFacetFictiousVertices(cell, j);
		Point&       p1      = cell->info();
	...
	template <class Tesselation> double Network<Tesselation>::surfaceSolidDoubleFictiousFacet(VertexHandle SV1, VertexHandle SV2, VertexHandle SV3)
	{
		double A[3], B[3];
	...
	template <class Tesselation> double Network<Tesselation>::surfaceSolidFacet(Sphere ST1, Sphere ST2, Sphere ST3)
	{
		double Area;
	...
	template <class Tesselation> void Network<Tesselation>::addBoundingPlane(CVector Normal, int id_wall)
	{
		...
		double pivot = Normal[Coordinate] < 0 ? cornerMax.x() * abs(Normal[0]) + cornerMax.y() * abs(Normal[1]) + cornerMax.z() * abs(Normal[2])
	...
	template <class Tesselation> void Network<Tesselation>::addBoundingPlane(Real center[3], double thickness, CVector Normal, int id_wall)
	{
		Tesselation& Tes = T[currentTes];
	...
	template <class Tesselation> double Network<Tesselation>::lineSolidFacet(Sphere ST1, Sphere ST2, Sphere ST3)
	{
		double  line;
	...
	template <class Tesselation> void Network<Tesselation>::setAlphaBoundary(double alpha, bool fixed)
	{
		...
		for (auto fp = facets.begin(); fp != facets.end(); fp++) {
			...
			if (as.classify(f.first) != AlphaShape::INTERIOR) {
				...
				if (as.is_infinite(outerCell) or fixed) {
					...
					if (std::isnan(sph.point().x()) or std::isnan(sph.point().y()) or std::isnan(sph.point().z())) {
