...
{
	if (facetVertices[i][0] == j) {
		i = facetVertices[i][1];
		j = facetVertices[i][2];
	} else if (facetVertices[i][1] == j) {
		i = facetVertices[i][0];
		j = facetVertices[i][2];
	} else {
		j = facetVertices[i][1];
...
namespace CGT {
	...
	public:
		...
		SimpleCellInfo& setScalar(const Real& scalar)
		{
			s = scalar;
			return *this;
		}
	...
	public:
		bool              isAlpha;
		bool              isFictious;
		SimpleVertexInfo& setVector(const CVector& u)
		{
			CVector::operator=(u);
			return *this;
		}
		SimpleVertexInfo& setFloat(const float& scalar)
		{
			s = scalar;
			return *this;
		}
		SimpleVertexInfo& setId(const unsigned int& id)
		{
			i = id;
			return *this;
		}
	...
	template <class vertex_info, class cell_info> class TriangulationTypes {
	public:
