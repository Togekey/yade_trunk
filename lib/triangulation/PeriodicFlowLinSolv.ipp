...
#define F77_FUNC(func) func##_
	...
	template <class _Tesselation> int PeriodicFlowLinSolv<_Tesselation>::setLinearSystem(Real dt)
	{
	...
	template <class _Tesselation> int PeriodicFlowLinSolv<_Tesselation>::setLinearSystemFullGS(Real dt)
	{
		...
		if (!isFullLinearSystemGSSet) {
			...
			for (int i = 1; i <= ncols; i++) {
				...
				if (!cell->info().Pcondition && !cell->info().isGhost) {
					//Add diagonal term
					fullAvalues[cell->info().index][4] = 1.f
					        / ((cell->info().kNorm())[0] + (cell->info().kNorm())[1] + (cell->info().kNorm())[2] + (cell->info().kNorm())[3]
					           + (fluidBulkModulus > 0 ? 1.f / (dt * fluidBulkModulus * cell->info().invVoidVolume()) : 0));
					//DUMP
					T_nnz++;
					for (int j = 0; j < 4; j++) {
