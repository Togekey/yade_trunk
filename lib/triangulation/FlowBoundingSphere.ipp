...
namespace CGT {
	...
	template <class Tesselation> void FlowBoundingSphere<Tesselation>::averageRelativeCellVelocity()
	{
		...
		for (FiniteCellsIterator cell = Tri.finite_cells_begin(); cell != cellEnd; cell++) {
			...
			if (cell->info().volume() == 0)
				cerr << "zero volume pore interrupting velocity calculation" << endl;
			else
	...
	template <class Tesselation> bool FlowBoundingSphere<Tesselation>::isOnSolid(double X, double Y, double Z)
	{
			...
			if (cell->info().fictious() == 0) {
				for (int i = 0; i < 4; i++) {
					velocityVolumes[cell->vertex(i)->info().id()] = velocityVolumes[cell->vertex(i)->info().id()]
					        + cell->info().averageVelocity() * std::abs(cell->info().volume());
	...
	template <class Tesselation> double FlowBoundingSphere<Tesselation>::getPorePressure(double X, double Y, double Z)
	{
		if (noCache && T[!currentTes].Max_id() <= 0)
	...
	template <class Tesselation> double FlowBoundingSphere<Tesselation>::getPoreTemperature(double X, double Y, double Z)
	{
		if (noCache && T[!currentTes].Max_id() <= 0)
			return 0; //the engine never solved anything
		RTriangulation& Tri  = T[noCache ? (!currentTes) : currentTes].Triangulation();
	...
	template <class Tesselation> int FlowBoundingSphere<Tesselation>::getCell(double X, double Y, double Z)
	{
		if (noCache && T[!currentTes].Max_id() <= 0) {
	...
	template <class Tesselation> void FlowBoundingSphere<Tesselation>::measurePressureProfile(double WallUpy, double WallDowny)
	{
		if (noCache && T[!currentTes].Max_id() <= 0)
			return; //the engine never solved anything
		RTriangulation& Tri = T[noCache ? (!currentTes) : currentTes].Triangulation();
	...
	template <class Tesselation> double FlowBoundingSphere<Tesselation>::averageSlicePressure(double Y)
	{
		RTriangulation& Tri   = T[currentTes].Triangulation();
		double          P_ave = 0.f;
		int             n     = 0;
	...
	template <class Tesselation> double FlowBoundingSphere<Tesselation>::averagePressure()
	{
		RTriangulation& Tri = T[currentTes].Triangulation();
...
#pragma omp parallel for num_threads(ompThreads)
		...
		for (int vn = 0; vn <= T[currentTes].maxId; vn++) {
			if (T[currentTes].vertexHandles[vn] == NULL)
				continue;
	...
	template <class Tesselation> void FlowBoundingSphere<Tesselation>::applyUserDefinedPressure(RTriangulation& Tri, vector<Real>& xpos, vector<Real>& pval)
	{
		...
		for (VCellIterator it = tmpCells.begin(); it != cellsEnd; it++) {
			...
			Point&      p1   = (*it)->info();
			CellHandle& cell = *it;
			if (p1.x() < xinit || p1.x() > xlast)
				cerr << "udef pressure: cell out of range" << endl;
			else {
				Real frac, intg;
...
#pragma omp parallel for num_threads(ompThreads > 0 ? ompThreads : 1)
				...
				if (!Tri.is_infinite(neighbourCell) && (neighbourCell->info().isvisited == ref || computeAllCells)) {
					...
					if (distance != 0) {
						if (minPermLength > 0 && distanceCorrection)
							distance = max(minPermLength * radius, distance);
						const CVector& Surfk         = cell->info().facetSurfaces[j];
						Real           area          = sqrt(Surfk.squared_length());
	...
	template <class Tesselation> vector<double> FlowBoundingSphere<Tesselation>::getConstrictions()
	{
		RTriangulation&            Tri = T[currentTes].Triangulation();
		...
		for (FiniteCellsIterator cell = Tri.finite_cells_begin(); cell != cellEnd; cell++) {
			if (cell->info().isGhost)
				continue; // retain only the cells with barycenter in the (0,0,0) period
			for (int j = 0; j < 4; j++) {
				neighbourCell = cell->neighbor(j);
	...
	template <class Tesselation> double FlowBoundingSphere<Tesselation>::computeEffectiveRadius(CellHandle cell, int j)
	{
		RTriangulation& Tri = T[currentTes].Triangulation();
		if (Tri.is_infinite(cell->neighbor(j)))
			return 0;
		...
		if (reff < 0)
			return 0; //happens very rarely, with bounding spheres most probably
	...
	template <class Tesselation>
	double FlowBoundingSphere<Tesselation>::computeEffectiveRadiusByPosRadius(
	        const Point& posA, const double& rA, const Point& posB, const double& rB, const Point& posC, const double& rC)
	{
		CVector B = posB - posA;
		...
		double A = ((pow(rA, 2)) * (1 - c1[0] / b1[0]) + ((pow(rB, 2) * c1[0]) / b1[0]) - pow(rC, 2) + pow(c1[0], 2) + pow(c1[1], 2)
	...
	template <class Tesselation> double FlowBoundingSphere<Tesselation>::computeEquivalentRadius(CellHandle cell, int j)
	{
	...
	template <class Tesselation> double FlowBoundingSphere<Tesselation>::computeHydraulicRadius(CellHandle cell, int j)
	{
		RTriangulation& Tri = T[currentTes].Triangulation();
		if (Tri.is_infinite(cell->neighbor(j)))
			return 0;
	...
	template <class Tesselation> void FlowBoundingSphere<Tesselation>::initializePressure(double pZero)
	{
		RTriangulation&     Tri     = T[currentTes].Triangulation();
		FiniteCellsIterator cellEnd = Tri.finite_cells_end();
...
#pragma omp parallel for
		...
		if (ppval && pxpos)
			applyUserDefinedPressure(Tri, *pxpos, *ppval);
		...
		for (unsigned int n = 0; n < imposedP.size(); n++) {
			...
			for (unsigned int kk = 0; kk < IPCells.size(); kk++) {
				if (cell == IPCells[kk])
					cerr << "Two imposed pressures fall in the same cell." << endl;
		...
		for (unsigned int n = 0; n < imposedF.size(); n++) {
			...
			for (unsigned int kk = 0; kk < IPCells.size(); kk++) {
				if (cell == IPCells[kk])
					cerr << "Both flux and pressure are imposed in the same cell." << endl;
				else if (cell->info().Pcondition)
	...
	template <class Tesselation> void FlowBoundingSphere<Tesselation>::initializeTemperatures(double tZero)
	{
		RTriangulation&     Tri     = T[currentTes].Triangulation();
		FiniteCellsIterator cellEnd = Tri.finite_cells_end();

		for (FiniteCellsIterator cell = Tri.finite_cells_begin(); cell != cellEnd; cell++) {
			if (!cell->info().Tcondition && !cell->info().isGhost && !cell->info().blocked)
				cell->info().temp() = tZero;
		}
		for (int bound = 0; bound < 6; bound++) {
			int& id = *boundsIds[bound];
	...
	template <class Tesselation> bool FlowBoundingSphere<Tesselation>::reApplyBoundaryConditions()
	{
		...
		if (ppval && pxpos)
			applyUserDefinedPressure(T[currentTes].Triangulation(), *pxpos, *ppval);
		for (unsigned int n = 0; n < imposedP.size(); n++) {
...
#pragma omp master
	...
	template <class Tesselation> double FlowBoundingSphere<Tesselation>::boundaryFlux(unsigned int boundaryId)
	{
		if (noCache && T[!currentTes].Max_id() <= 0)
			return 0;
	...
	template <class Tesselation> double FlowBoundingSphere<Tesselation>::boundaryArea(unsigned int boundaryId)
	{
		if (noCache && T[!currentTes].Max_id() <= 0)
			return 0;
		bool            tes = noCache ? (!currentTes) : currentTes;
	...
	template <class Tesselation> std::vector<std::vector<double>> FlowBoundingSphere<Tesselation>::boundaryVel(unsigned int boundaryId)
	{
	...
	template <class Tesselation> double FlowBoundingSphere<Tesselation>::getCavityFlux()
	{
...
#pragma omp parallel for
		...
		for (long i = 0; i < sizeCells; i++) {
			CellHandle& cell = Tes.cellHandles[i];
			if (!cell->info().isCavity || cell->info().isFictious || cell->info().blocked)
				continue; // only measuring cavity flux
			//cavityVolume += cell->info().volume();
			for (int j = 0; j < 4; j++) {
				if (cell->neighbor(j)->info().isCavity || cell->neighbor(j)->info().blocked)
					continue;
				cavityEdgeFlux
	...
	template <class Tesselation> void FlowBoundingSphere<Tesselation>::adjustCavityVolumeChange(double dt, int stepsSinceLastMesh, double /*pZero*/)
	{
...
#pragma omp parallel for
	...
	template <class Tesselation> void FlowBoundingSphere<Tesselation>::adjustCavityPressure(double dt, int stepsSinceLastMesh, double /*pZero*/)
	{
		double totalStep = dt * stepsSinceLastMesh;
		//double Q1=0;
		netCavityFlux             = 0;
...
#pragma omp parallel for
		...
		if (debugOut)
			cout << "flux added to cavity " << netCavityFlux << endl;
	}


	template <class Tesselation> void FlowBoundingSphere<Tesselation>::adjustCavityCompressibility(double pZero)
	{
		double sumP            = 0;
		int    numCells        = 0;
		netCavityFlux          = 0;
...
#pragma omp parallel for
		...
		for (long i = 0; i < sizeCells; i++) {
			...
			if (!controlCavityPressure)
				continue; // adjustCavPressure will track flux if active
			for (int j = 0; j < 4; j++) {
				if (cell->neighbor(j)->info().isCavity || cell->neighbor(j)->info().blocked)
					continue;
				netCavityFlux += (cell->info().kNorm())[j]
		...
		if (debugOut)
			cout << "Equivalent compressibility " << equivalentCompressibility << endl;
		...
		if (permeabilityMap) {
			...
			if (thermalEngine) {
				...
				for (unsigned kk = 0; kk < allIds.size(); kk++) {
					...
					if (isDrawable)
						vtkWrite.write_data(cellHandles[allIds[kk]]->info().temp());
				}
				vtkWrite.end_data();
				...
				for (unsigned kk = 0; kk < allIds.size(); kk++) {
					bool isDrawable = cellHandles[allIds[kk]]->info().isReal() && cellHandles[allIds[kk]]->vertex(0)->info().isReal()
					        ...
					        && cellHandles[allIds[kk]]->vertex(3)->info().isReal();
					if (isDrawable)
						vtkWrite.write_data(cellHandles[allIds[kk]]->info().Reynolds);
				}
				vtkWrite.end_data();
				...
				for (FiniteCellsIterator cell = Tri.finite_cells_begin(); cell != Tri.finite_cells_end(); ++cell) {
					bool isDrawable = cell->info().isReal() && cell->vertex(0)->info().isReal() && cell->vertex(1)->info().isReal()
					        && cell->vertex(2)->info().isReal() && cell->vertex(3)->info().isReal();
					if (isDrawable) {
						vtkWrite.write_data(cell->info().Tcondition);
					}
				}
				vtkWrite.end_data();
			}
			...
			for (FiniteCellsIterator cell = Tri.finite_cells_begin(); cell != Tri.finite_cells_end(); ++cell) {
				bool isDrawable = cell->info().isReal() && cell->vertex(0)->info().isReal() && cell->vertex(1)->info().isReal()
				        && cell->vertex(2)->info().isReal() && cell->vertex(3)->info().isReal();
				if (isDrawable) {
					vtkWrite.write_data(cell->info().isCavity);
				}
			}
			vtkWrite.end_data();
			...
			for (FiniteCellsIterator cell = Tri.finite_cells_begin(); cell != Tri.finite_cells_end(); ++cell) {
				bool isDrawable = cell->info().isReal() && cell->vertex(0)->info().isReal() && cell->vertex(1)->info().isReal()
				        && cell->vertex(2)->info().isReal() && cell->vertex(3)->info().isReal();
				if (isDrawable) {
					vtkWrite.write_data(cell->info().isAlpha);
				}
			}
			vtkWrite.end_data();
			...
			for (FiniteCellsIterator cell = Tri.finite_cells_begin(); cell != Tri.finite_cells_end(); ++cell) {
				bool isDrawable = cell->info().isReal() && cell->vertex(0)->info().isReal() && cell->vertex(1)->info().isReal()
				        && cell->vertex(2)->info().isReal() && cell->vertex(3)->info().isReal();
				if (isDrawable) {
					vtkWrite.write_data(cell->info().Pcondition);
				}
			}
			vtkWrite.end_data();
	...
	*/
		...
		for (FiniteCellsIterator cell = Tri.finite_cells_begin(); cell != Tri.finite_cells_end(); ++cell) {
			...
			if (isDrawable) {
				...
				if (minId != 0)
					vtkfile.write_cell(vertexIdMap[id0], vertexIdMap[id1], vertexIdMap[id2], vertexIdMap[id3]);
				else
	...
	template <class Tesselation>
	double
	FlowBoundingSphere<Tesselation>::samplePermeability(double& xMin, double& xMax, double& yMin, double& yMax, double& zMin, double& zMax /*, string key*/)
	{
	...
	template <class Tesselation> bool FlowBoundingSphere<Tesselation>::isInsideSphere(double& x, double& y, double& z)
	{
		RTriangulation& Tri = T[currentTes].Triangulation();
		for (FiniteVerticesIterator vIt = Tri.finite_vertices_begin(); vIt != Tri.finite_vertices_end(); vIt++) {
			double radius = vIt->point().weight();
			if (pow((x - (vIt->point()[0])), 2) + pow((y - (vIt->point()[1])), 2) + pow((z - (vIt->point()[2])), 2) <= radius)
				return true;
		}
		return false;
	}
	...
	template <class Tesselation> double FlowBoundingSphere<Tesselation>::fractionalSolidArea(CellHandle cell, int j)
	{
		double area;
		int    k = 0, l = 0, m = 0;
		if (j == 0) {
