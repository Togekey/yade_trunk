...
namespace CGT {
	...
	void KinematicLocalisationAnalyser::SetBaseFileName(string name) { base_file_name = name; }
	...
	void KinematicLocalisationAnalyser::SetConsecutive(bool t) { consecutive = t; }
	...
	KinematicLocalisationAnalyser::Oriented_Filtered_edges(Real Nymin, Real Nymax, vector<Edge_iterator>& filteredList)
	{
	...
	Real KinematicLocalisationAnalyser::Filtered_volume(TriaxialState& /*state*/) { return 0; }

	Real KinematicLocalisationAnalyser::Contact_coordination(TriaxialState& state) { return Filtered_contacts(state) / Filtered_grains(state); }

	Real KinematicLocalisationAnalyser::Neighbor_coordination(TriaxialState& state) { return Filtered_neighbors(state) / Filtered_grains(state); }
	...
	Tenseur_sym3 KinematicLocalisationAnalyser::Neighbor_fabric(TriaxialState& state)
	{
		...
		for (Edge_iterator ed_it = T.edges_begin(); ed_it != ed_end; ++ed_it) {
			if (!T.is_infinite(*ed_it)) {
				s = T.segment(*ed_it);
				if (state.inside(s.source()) && state.inside(s.target())) {
					v = T.segment(*ed_it).to_vector() * (1 / sqrt(T.segment(*ed_it).squared_length()));
					for (int i = 1; i < 4; i++)
						for (int j = 3; j >= i; j--)
	...
	Tenseur_sym3 KinematicLocalisationAnalyser::Contact_fabric(TriaxialState& state)
	{
		Tenseur_sym3                   Tens;
		CVector                        v;
		TriaxialState::ContactIterator cend = state.contacts_end();
	...
	vector<pair<Real, Real>>& KinematicLocalisationAnalyser::NormalDisplacementDistribution(vector<Edge_iterator>& edges, vector<pair<Real, Real>>& row)
	{
	...
	ofstream& KinematicLocalisationAnalyser::NormalDisplacementDistributionToFile(vector<Edge_iterator>& edges, ofstream& output_file)
	{
		...
		//output part :
		output_file << "#Normal displacement distribution" << endl
		            << "eps3=" << Delta_epsilon(3, 3) << " eps2=" << Delta_epsilon(2, 2) << " eps1=" << Delta_epsilon(1, 1)
		            << " number of neigbors: " << edges.size() << endl
		            << "Un_min=" << 1.5 * row[0].first - 0.5 * row[1].first << " Un_max=" << row[row.size() - 1].first << endl;
		cout << "#Normal displacement distribution" << endl
		     << "eps3=" << Delta_epsilon(3, 3) << " eps2=" << Delta_epsilon(2, 2) << " eps1=" << Delta_epsilon(1, 1)
		     << " number of neigbors: " << edges.size() << endl
		     << "Un_min=" << 1.5 * row[0].first - 0.5 * row[1].first << " Un_max=" << row[row.size() - 1].first << endl;
	...
	ofstream& KinematicLocalisationAnalyser::ContactDistributionToFile(ofstream& output_file)
	{
		...
		for (int i = 0; i <= sphere_discretisation; ++i)
			row[i].second *= normalize;

		//output part :
		output_file << "#Contacts distribution" << endl
		            << "(filter dist. = " << (*TS1).filter_distance << ", " << nc1 << " contacts, " << nc2 << " excluded contacts, for " << ng1 << "/"
		            << (ng1 + ng2) << " grains)" << endl;
		output_file << "max_nz number_of_contacts" << endl;
		cerr << "#Contacts distribution (filter dist. = " << (*TS1).filter_distance << ", " << nc1 << " contacts, " << nc2 << " excluded contacts, for "
		     << ng1 << "/" << (ng1 + ng2) << " grains)" << endl;
	...
	ofstream& KinematicLocalisationAnalyser::AllNeighborDistributionToFile(ofstream& output_file)
	{
		vector<pair<Real, Real>> row;
		...
		output_file << "#Neighbors distribution" << endl
		            << "(filter dist. = " << (*TS1).filter_distance << ", " << nv1 << " neighbors + " << nv2 << " excluded + " << nv3
		            << " infinite, for " << ng1 << "/" << (ng1 + ng2) << " grains)" << endl;
		output_file << "max_nz number_of_neighbors" << endl;
		cerr << "#Neighbors distribution" << endl
		     << "(filter dist. = " << (*TS1).filter_distance << ", " << nv1 << " neighbors + " << nv2 << " excluded + " << nv3 << " infinite, for "
		     << ng1 << "/" << (ng1 + ng2) << " grains)" << endl;
	...
	void KinematicLocalisationAnalyser::SetForceIncrements(
	        void) //WARNING : This function will modify the contact lists : add virtual (lost)) contacts in state 1 and modify old_force and force in state 0, execute this function after all other force analysis functions if you want to avoid problems
	{
		...
		for (int i = 0; i < Nc0; ++i) {
			...
			if (TS0->contacts[i]->status != TriaxialState::Contact::LOST) {
				//  cerr << 2;
				for (int j = 0; j < Nc1; ++j) {
					if (TS0->contacts[i]->grain1->id == TS1->contacts[j]->grain1->id
					    && TS0->contacts[i]->grain2->id
					            == TS1->contacts[j]->grain2->id) { // This is a PERSISTENT contact (i.e. it is present in state 0 and 1)
	...
	ofstream& KinematicLocalisationAnalyser::StrictNeighborDistributionToFile(ofstream& output_file) { return output_file; }
	...
	CVector KinematicLocalisationAnalyser::Deplacement(Finite_cells_iterator cell, int facet)
	{
		...
		for (int i = 0; i < 4; i++) {
			...
			if (i != facet) {
				...
				CVector meanFieldDisp
				        = CVector(TS0->grain(id).sphere.point().x(), TS0->grain(id).sphere.point().y(), TS0->grain(id).sphere.point().z())
				        - fixedPoint;
				if (1) { //fluctuations
	...
	{
		CVector S = cross_product(
		                    (cell->vertex(l_vertices[facet][1])->point().point()) - (cell->vertex(l_vertices[facet][0])->point().point()),
		                    (cell->vertex(l_vertices[facet][2])->point().point()) - (cell->vertex(l_vertices[facet][1])->point().point()))
		        / 2.f;
	...
	Real KinematicLocalisationAnalyser::computeMacroPorosity(void) { return (1 - v_solid_total / (TS1->haut * TS1->larg * TS1->prof)); }
