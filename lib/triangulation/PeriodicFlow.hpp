...
namespace CGT {
	...
	template <class _Tesselation> class PeriodicFlow : public CGT::FlowBoundingSphere<_Tesselation> {
	public:
	...
	template <class _Tesselation> void PeriodicFlow<_Tesselation>::interpolate(Tesselation& Tes, Tesselation& NewTes)
	{
		CellHandle      oldCell;
	...
	template <class _Tesselation> void PeriodicFlow<_Tesselation>::computeFacetForcesWithCache(bool onlyCache)
	{
		...
		for (unsigned int k = 0; k < 3; k++)
			pDeltas[k] = CellInfo::hSize[k] * CellInfo::gradP;
		...
		for (VCellIterator cellIt = T[currentTes].cellHandles.begin(); cellIt != T[currentTes].cellHandles.end(); cellIt++) {
			...
			for (int yy = 0; yy < 4; yy++) {
				...
				T[currentTes].vertexHandles[vhi.id()]->info().forces
				        = T[currentTes].vertexHandles[vhi.id()]->info().forces + cell->info().unitForceVectors[yy] * unshiftedP;
			}
		}
		if (debugOut) {
	...
	template <class _Tesselation> void PeriodicFlow<_Tesselation>::computePermeability()
	{
		...
		for (VCellIterator cellIt = T[currentTes].cellHandles.begin(); cellIt != T[currentTes].cellHandles.end(); cellIt++) {
			...
			for (int j = 0; j < 4; j++) {
				...
				if (!Tri.is_infinite(neighbourCell) /*&& (neighbour_cell->info().isvisited==ref || computeAllCells)*/) {
					...
					assert((W[permut3[jj][1]]->point() - W[permut3[jj][0]]->point())
					                       * (W[permut3[jj][2]]->point() - W[permut3[jj][0]]->point())
					               >= 0
					       && (W[permut3[jj][1]]->point() - W[permut3[jj][0]]->point())
					                       * (W[permut3[jj][2]]->point() - W[permut3[jj][0]]->point())
					               <= 1);
					for (int jj = 0; jj < 3; jj++)
						cell->info().facetSphereCrossSections[j][jj] = 0.5 * W[jj]->point().weight()
						        * Wm3::FastInvCos1((W[permut3[jj][1]]->point() - W[permut3[jj][0]]->point())
						                           * (W[permut3[jj][2]]->point() - W[permut3[jj][0]]->point()));
#else
					...
					if (distance != 0) {
						...
						if (k < 0 && debugOut) {
							...
							cout << "__ k<0 __" << k << " "
							     ...
							     << W[1]->info().id() << " " << W[2]->info().id() << " " << p1 << " " << p2 << " test " << test
							     << endl;
						}
					} else {
						cout << "infinite K2! surfaces will be missing (FIXME)" << endl;
						k = infiniteK;
					}
		...
		meanRadius /= pass;
		meanDistance /= pass;
		Real globalK = kFactor * meanDistance * vPoral
						...
						if (!neighbourCell->info().isGhost)
							...
							for (int ii = 0; ii < 4; ii++)
								if (true_neighbourCell->neighbor(ii)->info().index == cell->info().index) {
									(true_neighbourCell->info().kNorm())[ii] = (cell->info().kNorm())[j];
									break;
								}
						}
					}
				...
				if (!cell->info().Pcondition && !cell->info().isGhost) {
					...
					if (n != 0 || j != 0) {
						...
						if (compressible) {
							...
							cell->info().setP(
							        (-(cell->info().dv() - m) * cell->info().invSumK - cell->info().p()) * relax
							        + cell->info().shiftedP());
						}
					}
					dp -= cell->info().p();
