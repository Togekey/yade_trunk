...
void Subdomain::setCommunicationContainers()
{
	...
	for (unsigned int i = 1; i != intersections.size(); ++i) {
		if ((intersections[i].size() == zero) || (i == static_cast<unsigned int>(subdomainRank))) {
			continue;
...
Real Subdomain::boundOnAxis(
        Bound&   b,
        Vector3r direction,
        bool     min) //return projected extremum of an AABB in a particular direction (in the the '+' or '-' sign depending on 'min' )
{
	Vector3r size     = b.max - b.min;
	Real     extremum = 0;
	for (unsigned k = 0; k < 3; k++)
		extremum += std::abs(size[k] * direction[k]); // this is equivalent to taking the vertex maximizing projected length
	if (min)
		extremum = -extremum;
	extremum
	        += (b.max + b.min)
