...
void Scene::moveToNextTimeStep()
{
	...
	if (!subStepping && subStep < 0) {
		...
		if (TimingInfo::enabled) {
			TimingInfo::enabled = false;
			LOG_INFO("O.timingEnabled disabled, since O.subStepping is used.");
		}
		if (subStep < -1 || subStep > (int)engines.size()) {
			LOG_ERROR("Invalid value of Scene::subStep (" << subStep << "), setting to -1 (prologue will be run).");
			subStep = -1;
		}
		...
		if (!subStepping) {
			maxSubStep = engines.size();
			LOG_INFO("Running remaining sub-steps (" << subStep << "…" << maxSubStep << ") before disabling sub-stepping.");
		}
		for (int subs = subStep; subs <= maxSubStep; subs++) {
			...
			// ** 3. ** epilogue
				...
				iter++;
				time += dt; /* gives -1 along with the increment afterwards */
				subStep = -2;
			}
			// (?!)
			else { /* never reached */
	...
	for (const auto& b : *bodies) {
		...
		if (!b->material->stateTypeOk(b->state.get())) {
			throw std::runtime_error(
			        "Body #" + boost::lexical_cast<string>(b->getId()) + ": Body::material type " + b->material->getClassName()
			        + " doesn't correspond to Body::state type " + b->state->getClassName() + " (should be "
			        + b->material->newAssocState()->getClassName() + " instead).");
		}
	}
}
...
void Scene::updateBound()
{
	...
	for (const auto& b : *bodies) {
		...
		if (b->bound) {
			for (int i = 0; i < 3; i++) {
				if (!std::isinf(b->bound->max[i]))
					mx[i] = max(mx[i], b->bound->max[i]);
				if (!std::isinf(b->bound->min[i]))
