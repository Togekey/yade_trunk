...
void Clump::add(const shared_ptr<Body>& clumpBody, const shared_ptr<Body>& subBody)
{
	...
	if (clump->members.count(subId) != 0)
		throw std::invalid_argument(
		        ("Body #" + boost::lexical_cast<string>(subId) + " is already part of this clump #" + boost::lexical_cast<string>(clumpBody->id))
		                .c_str());
	if (subBody->isClumpMember())
		throw std::invalid_argument(
		        ("Body #" + boost::lexical_cast<string>(subId) + " is already a clump member of #" + boost::lexical_cast<string>(subBody->clumpId))
		                .c_str());
...
void Clump::del(const shared_ptr<Body>& clumpBody, const shared_ptr<Body>& subBody)
{
	...
	const shared_ptr<Clump> clump = YADE_PTR_CAST<Clump>(clumpBody->shape);
	if (clump->members.erase(subBody->id) != 1)
		throw std::invalid_argument(("Body #" + boost::lexical_cast<string>(subBody->id) + " not part of clump #"
		                             + boost::lexical_cast<string>(clumpBody->id) + "; not removing.")
		                                    .c_str());
...
void Clump::updateProperties(const shared_ptr<Body>& clumpBody, unsigned int discretization)
{
	...
	*/
	...
	Matrix3r Ic_orientG = inertiaTensorTranslate(
	        Ig, -M /* negative mass means towards centroid */, state->pos); // inertia at clump's centroid but with world orientation
...
void Clump::updatePropertiesNonSpherical(const shared_ptr<Body>& clumpBody, bool intersecting, shared_ptr<Scene> rb)
{ //FIXME
	...
	*/
	...
	// this will calculate translation only, since rotation is zero
	Matrix3r Ic_orientG = Clump::inertiaTensorTranslate(
	        Ig, -M /* negative mass means towards centroid */, state->pos); // inertia at clump's centroid but with world orientation
	//TRWM3MAT(Ic_orientG);
...
void Clump::updatePropertiesNonSpherical(const shared_ptr<Body>& clumpBody, bool intersecting)
{ //FIXME
	//LOG_DEBUG("Updating clump #"<<getId()<<" parameters");
	//assert(members.size()>0);
	const shared_ptr<State> state(clumpBody->state);
...
void Clump::addNonSpherical(const shared_ptr<Body>& clumpBody, const shared_ptr<Body>& subBody)
{ //FIXME
	Body::id_t subId = subBody->getId();
	if (subBody->clumpId != Body::ID_NONE)
		throw std::invalid_argument(
		        ("Body #" + boost::lexical_cast<string>(subId) + " is already in clump #" + boost::lexical_cast<string>(subBody->clumpId)).c_str());
	const shared_ptr<Clump> clump = YADE_PTR_CAST<Clump>(clumpBody->shape);
	if (clump->members.count(subId) != 0)
		throw std::invalid_argument(
		        ("Body #" + boost::lexical_cast<string>(subId) + " is already part of this clump #" + boost::lexical_cast<string>(clumpBody->id))
