			...
			if (isDoubleTwist) {
				...
				if (strainStressValuesDT[0](0) == 0. && strainStressValuesDT[0](1))
					throw std::invalid_argument(
					        "WireMat.strainStressValuesDT: Definition must start with values greater than zero (strain>0,stress>0)");
			}
			break;
		case 2:
			...
			break;
		default: throw std::invalid_argument("WireMat.type: Type must be 0, 1 or 2."); break;
	}
}
...
bool Law2_ScGeom_WirePhys_WirePM::go(shared_ptr<IGeom>& ig, shared_ptr<IPhys>& ip, Interaction* contact)
{
	...
	Real limitFactor = 0.;
	if (Fn < 0.)
		limitFactor = std::abs(D / (DFValues.back()(0)));
