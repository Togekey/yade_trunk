...
bool SimpleShear::generate(std::string& /*message*/)
{
	...
	YADE_PTR_CAST<FrictMat>(w2->material)->frictionAngle
	        = matFrictionDeg * Mathr::PI / 180.0; // so that we have phi(spheres-inferior wall)=phi(sphere-sphere)
	...
	YADE_PTR_CAST<FrictMat>(w4->material)->frictionAngle
	        = matFrictionDeg * Mathr::PI / 180.0; // so that we have phi(spheres-superior wall)=phi(sphere-sphere)
	...
	createBox(
	        w5,
	        Vector3r(length / 2.0, height / 2.0, -width / 2.0 - thickness / 2.0),
	...
	string out = GenerateCloud(
	        sphere_list,
	        Vector3r(0, 0, -width / 2.0),
	        Vector3r(length, height, width / 2.0),
	        1000,
	        0.3,
	        0.7); // tries to generate a sample of 1000 spheres, with a required porosity of 0.7
	cout << out << endl;
	...
	for (i = 0; i < number; ++i) {
		...
		if (t == tries) {
			string str1 = "Generated a sample with " + boost::lexical_cast<string>(i) + " spheres inside box of dimensions: ("
			        + boost::lexical_cast<string>(dimensions[0]) + "," + boost::lexical_cast<string>(dimensions[1]) + ","
...
std::pair<string, bool> SimpleShear::ImportCloud(vector<BasicSphere>& sphere_list, string importFilename)
{
	...
	if (importFilename.size() != 0 && boost::filesystem::exists(importFilename)) {
		ifstream loadFile(
		        importFilename.c_str()); // cree l'objet loadFile de la classe ifstream qui va permettre de lire ce qu'il y a dans importFilename
