...
Real Law2_ScGeom_MindlinPhys_Mindlin::normElastEnergy()
{
	...
	FOREACH(const shared_ptr<Interaction>& I, *scene->interactions)
	{
		...
		if (phys) {
			if (includeAdhesion) {
				normEnergy += (std::pow(scg->penetrationDepth, 5. / 2.) * 2. / 5. * phys->kno - phys->adhesionForce * scg->penetrationDepth);
			} else {
				normEnergy += std::pow(scg->penetrationDepth, 5. / 2.) * 2. / 5. * phys->kno;
