...
bool BlockGen::generate(string& /*message*/)
{
	...
	if (boundaries) {
		...
		int                            jointNo = 0; //double persistenceA=0; double persistenceB=0; double spacing = 0;
		while (file.good()) {
			count++;
			...
			if (count == 3)
				jointCentre.x() = valueDouble;
			if (count == 4)
				jointCentre.y() = valueDouble;
			if (count == 5) {
				jointCentre.z()           = valueDouble;
				joint[jointNo].centre     = jointCentre;
				joint[jointNo].isBoundary = true;
			}
			if (count == 6) {
				joint[jointNo].phi_b = valueDouble;
			}
			if (count == 7) {
				joint[jointNo].phi_r = valueDouble;
			}
			if (count == 8) {
				joint[jointNo].JRC = valueDouble;
			}
			if (count == 9) {
				joint[jointNo].JCS = valueDouble;
			}
			if (count == 10) {
				joint[jointNo].asperity = valueDouble;
			}
			if (count == 11) {
				joint[jointNo].sigmaC = valueDouble;
			}
			if (count == 12) {
				joint[jointNo].cohesion = valueDouble;
			}
			if (count == 13) {
				joint[jointNo].tension = valueDouble;
			}
			if (count == 14) {
				joint[jointNo].lambda0 = valueDouble;
			}
			if (count == 15) {
				joint[jointNo].heatCapacity = valueDouble;
			}
			if (count == 16) {
				joint[jointNo].hwater       = valueDouble;
	...
	if (persistentPlanes) {
		...
		/* int boundaryNo = 0; int boundaryCount = 0; int abdCount=0; */ Vector3r planeNormal(
		        0, 0, 0); /* int jointNo = 0; double persistenceA=0; double persistenceB=0; */
		double spacing = 0;
		double a = 0, b = 0, c = 0 /* d */, l = 0.0;
		while (file.good()) {
			count++;
			getline(file, value, ';'); // read a string until next comma: http://www.cplusplus.com/reference/string/getline/
			//std::cout<<"count: "<<count<<", value: "<<value<<endl;
			...
			if (count == 8) {
				asperity = valueDouble;
			}
			if (count == 9) {
				sigmaC = valueDouble;
			}
			if (count == 10) {
				cohesion = valueDouble;
			}
			if (count == 11) {
				tension = valueDouble;
				...
				for (int j = 1; j <= number / 2.0 /* -1 */; j++) {
					...
					joint[i].b        = b / l;
					joint[i].c        = c / l;
					joint[i].d        = spacing * static_cast<double>(j);
					joint[i].phi_b    = phi_b;
					joint[i].phi_r    = phi_r;
					joint[i].JRC      = JRC;
					joint[i].JCS      = JCS;
	...
	if (jointProbabilistic) {
		...
		while (file.good()) {
			...
			if (count == 6) {
				...
				joint[jointNo].persistence_c.push_back(-rotatedPersistence.z());
				joint[jointNo].persistence_d.push_back(valueDouble);
				joint[jointNo].persistence_d.push_back(valueDouble);
			}
			if (count == 7) {
				double   strike    = dipdir2 - 90.0;
	...
	if (opening) {
		...
		while (file.good()) {
			...
			double      valueDouble = atof(valueChar);
			if (count == 1) {
				planeNormal.x() = valueDouble;
			}
			if (count == 2) {
				planeNormal.y() = valueDouble;
			}
			if (count == 3) {
				planeNormal.z() = valueDouble;
		...
		for (int i = 0; i < blkOriCount; i++) {
			...
			do {
				...
				if (joint[j].throughGoing == false) {
					double twoRadiiDist
					        = 1.2 * (presentBlock.R + std::max(fabs(joint[j].persistence_d[0]), fabs(joint[j].persistence_d[2])));
				...
				if (contactDetectionLPCLPglobal(joint[j], presentBlock, vertexFrJoint) && presentBlock.tooSmall == false) {
					...
					if (joint[j].isBoundary == true) {
						Vector3r startingPt = Vector3r(0, 0, 0); //centroid
						//						//bool converge=startingPointFeasibility(blkA, startingPt);
						double radius = inscribedSphereCLP(
						        blkA,
						        startingPt,
						...
						double   fA        = joint[j].a * (centroidA.x() - joint[j].centre.x())
					...
					if (joint[j].throughGoing == false) {
						...
						double   chebyshevRa = inscribedSphereCLP(blkA, tempA, twoDimension);
						...
						double   chebyshevRb = inscribedSphereCLP(blkB, tempB, twoDimension);
...
bool BlockGen::createBlock(shared_ptr<Body>& body, struct Block block, int number)
{
	...
	//	Real maxYoverall = 1.00*std::max(fabs(maxY),fabs(minY));
	//	Real maxZoverall = 1.00*std::max(fabs(maxZ),fabs(minZ));
	//if(number == 482 ){
	...
	for (unsigned int k = 0; k < block.a.size(); k++) {
		block.d[k] = -1.0
		        * (block.a[k] * (centroid.x() - prevCentre.x()) + block.b[k] * (centroid.y() - prevCentre.y())
		           + block.c[k] * (centroid.z() - prevCentre.z()) - block.d[k]);
	}
	...
	//	maxXoverall = 1.00*std::max(fabs(maxX),fabs(minX));
	//	maxYoverall = 1.00*std::max(fabs(maxY),fabs(minY));
	//	maxZoverall = 1.00*std::max(fabs(maxZ),fabs(minZ));
	...
	//FIXME: Setting the Inertia values below equal to zero when they are too small should be revised or removed. Choosing different combination of units, the inertia values could get quite small values. Alternativelly, we can perform the check below by normalising the inertia values to the dimensions of the particle, i.e. by dividing them with a multiple of "R"
	...
	if (exactRotation == true) {
		...
		double maxInertia = std::max(
		        std::max(std::max(lapackEigenVal(0, 0), lapackEigenVal(1, 1)), lapackEigenVal(2, 2)),
	...
	for (int i = 0; i < planeNo; i++) {
		...
		plane2 = q * plane2;
		pBlock->a.push_back(plane2.x());
		block.a[i] = plane2.x();
		pBlock->b.push_back(plane2.y());
		block.b[i] = plane2.y();
		pBlock->c.push_back(plane2.z());
		block.c[i] = plane2.z();
	...
	if (color[0] == -1 or color[1] == -1 or color[2] == -1) {
		pBlock->color = Vector3r(
		        Mathr::UnitRandom(), Mathr::UnitRandom(), Mathr::UnitRandom()); //std::max(std::max(maxXoverall, maxYoverall),maxZoverall) ; //
	} else {
		pBlock->color = color;
	}
			...
			for (int k = j + 1; k < planeNo; k++) {
				...
				if (fabs(det) > pow(10, -15)) {
					...
					if (info2 != 0) {
						...
						bool     inside = true;
						Vector3r vertex(D[0], D[1], D[2]);

						for (int l = 0; l < planeNo; l++) {
							Real plane = pBlock->a[l] * vertex.x() + pBlock->b[l] * vertex.y() + pBlock->c[l] * vertex.z()
							        - pBlock->d[l] - pBlock->r;
							if (plane > pow(10, -3)) {
								inside = false;
							}
						...
						if (inside == true) {
							...
							if (vertCount == 0) { // Allow the first vertex to be written
								pBlock->vertices.push_back(vertex);
								vertCount = vertCount + 1;
							} else {
								minDistance = 1.0e15;
								for (unsigned int n = 0; n < vertCount; n++) {
									Distance = (vertex - pBlock->vertices[n]).norm();
									if (Distance < minDistance) {
										minDistance = Distance;
									}
								}

								if (minDistance / (pBlock->R > 0.0 ? pBlock->R : 1.0) > 1.0e-6) {
									pBlock->vertices.push_back(vertex);
									vertCount = vertCount + 1;
								}
							}

							int vertexID = pBlock->vertices.size() - 1;
...
void BlockGen::calculateInertia(struct Block block, Real& Ixx, Real& Iyy, Real& Izz, Real& Ixy, Real& Ixz, Real& Iyz)
{
			...
			for (int k = j + 1; k < planeNo; k++) {
				...
				if (fabs(det) > pow(10, -15)) {
					...
					if (info != 0) {
						...
						for (int l = 0; l < planeNo; l++) {
							Real plane = block.a[l] * vertex.x() + block.b[l] * vertex.y() + block.c[l] * vertex.z() - block.d[l]
							        - block.r;
							if (plane > pow(10, -3)) {
								inside = false;
							}
						...
						if (inside == true) {
							...
							if (vertCount == 0) { // Allow the first vertex to be written
								...
								if (minDistance / (block.R > 0.0 ? block.R : 1.0) > 1.0e-6) {
									vertices.push_back(vertex);
									vertCount = vertCount + 1;
								}
							}
	...
	vector<Vector3r> verticesOnPlane;
	vector<Vector3r> oriVerticesOnPlane;
	for (unsigned int j = 0; j < block.a.size(); j++) {
		...
		vector<Vector3r> oriOrderedVerticesOnPlane;
		unsigned int     h   = 0;
		unsigned int     k   = 1;
		unsigned int     m   = 2;
		...
		Vector3r        baseOnPolygon(0, 0, 0);
		Vector3r        oriBaseOnPolygon(0, 0, 0);
		...
		for (int i = 0; i < vertexOnPlane.rows() - 1; i++) {
			...
			// tempVert1,2,3,4: vertices of each tetrahedron
