        ...
        const shared_ptr<Interaction>& c)
{
	...
	if (stepBisection < pow(10, -6)) {
		std::cout << "R1: " << s1->R << ", R2: " << s2->R << ", stepBisection: " << stepBisection << ", id1: " << c->getId1()
		          << ", id2: " << c->getId2() << endl;
	}
	...
	bool hasPhys = false;
	if (c->phys) {
		phys     = YADE_PTR_CAST<KnKsPBPhys>(c->phys);
		hasPhys  = true;
		shearDir = phys->shearDir; //normalization should take place in the Contact Law
	}
	...
	if (s1->isLining == true || s2->isLining == true) {
		...
		if (s1->isLining == true) {
			...
			fA        = evaluatePB(
                                cm2,
                                state2,
                                Vector3r(0, 0, 0) /* shift2*/,
                                contactPt); //Periodic Boundary conditions are not considered for the RockLiningGlobal code
			if (fA < 0.0) {
				contact   = true;
		...
		if (contact == true || c->isReal() || force) {
			if (contact) {
				...
				if (hasPhys) {
					...
					/*phys->ptOnP1 = contactPt; */ phys->ptOnP1 = ptOnP1;
					phys->ptOnP2                                = ptOnP2; // phys->initial1 = scm->contactPoint;
	...
	if (contact /* && convergeFeasibility == true */) {
		...
		if (converge == false) {
			...
			contact = false;
			std::cout << "One outside, fA: " << fA << ", fB: " << fB << ",s1->isBoundary: " << s1->isBoundary
			          << ", s2->isBoundary: " << s2->isBoundary << endl;
			contactPt = 0.5 * (state1.pos + state2.pos);
		}
	}
	...
	if (contact == true || c->isReal() || force) {
		if (contact) {
			normalP1 = getNormal(cm1, state1, Vector3r(0, 0, 0), contactPt, twoDimension);
			normalP1.normalize();
			normalP2 = getNormal(cm2, state2, shift2, contactPt, twoDimension);
			normalP2.normalize();
			...
			if (hasPhys) {
				...
				double jointLength    = phys->jointLength;
				int    smallerID      = 1; //bool twoD = phys->twoDimension;
				...
				if (twoDimension) { //TODO: We can move this inside the getAreaPolygon2, in the 2D branch: the unitWidth2D parameter will have to become variable in the getAreaPolygon2 function
					if (jointLength < pow(10, -11) || std::isnan(jointLength) || calJointLength == false) {
						jointLength = 1.0; /*std::min(s1->R,s2->R);*/
					} //FIXME: Setting the jointLength equal to 1.0 if it is too small is not a good idea. We can alternativelly leave the jointLength as it is, decide a very small default value, set it equal to its previous value or to the distance "R" as in the comment above. But in any case, we need to output a warning as well, so that the user knows what's happening in the simulation.
				...
				if (phys->useFaceProperties) {
					double phi_b1 = 0.0, phi_b2 = 0.0, phi_r1 = 0.0, phi_r2 = 0.0, cohesion1 = 0.0, cohesion2 = 0.0, tension1 = 0.0,
					       tension2  = 0.0;
					...
					double f1 = evaluatePhys(
					        ...
					        phi_r1,
					        /* JRC1, JSC1, */ cohesion1,
					        /* sigmaC1, asperity1, */ tension1,
					        /* lambda01, heatCapacity1, hwater1, */ intactRock1,
					        noActive1,
					        jointType1);
					double f2 = evaluatePhys(
					        cm2,
					        state2,
					        shift2,
					        contactPt,
					        ...
					        phi_r2,
					        /* JRC2, JSC2, */ cohesion2,
					        /* sigmaC2, asperity2, */ tension2,
					        /* lambda02, heatCapacity2, hwater2, */ intactRock2,
					        noActive2,
					        jointType2);

					f1 += 0;
					f2 += 0; //FIXME: These two statements are added to mute compiler warnings for unused parameters f1, f2. To be amended soon, by changing the type of evaluatePhys to "void", instead of "double"
...
double Ig2_PB_PB_ScGeom::evaluatePhys(
        const shared_ptr<Shape>&                                          cm1,
        const State&                                                      state1,
        const Vector3r&                                                   shift2,
        ...
        double&                                                           phi_b,
        double&                                                           phi_r,
        /* double& JRC, double& JSC, */ double&                           cohesion,
        /* double& sigmaC, double& asperity, */ double&                   tension,
        /* double& lambda0,double& heatCapacity, double &hwater, */ bool& intactRock,
        int&                                                              activePlanesNo,
        int&                                                              jointType)
{
	PotentialBlock* s1 = static_cast<PotentialBlock*>(cm1.get());
	...
	Real f = pSum2
	        - pow(r, 2); //FIXME: In this function we calculate pSum2 and f but we don't use them. The type of the function will be modified to "void".
...
/* ***************************************************************************************************************************** */
double Ig2_PB_PB_ScGeom::getAreaPolygon2(
        const shared_ptr<Shape>& cm1,
        const State&             state1,
        const shared_ptr<Shape>& cm2,
        const State&             state2,
        ...
        const bool               twoD)
{
	...
	if (!twoD) { //3D contact - search counter-clockwise
		...
		area1(0, 0) = ptOnBoundary.x();
		area2(0, 0) = ptOnBoundary.y();
		area3(0, 0) = ptOnBoundary.z();
		area1(1, 0) = ptOnBoundary.y();
		area2(1, 0) = ptOnBoundary.z();
		area3(1, 0) = ptOnBoundary.x();
		area1(2, 0) = 1.0;
		area2(2, 0) = 1.0;
		area3(2, 0) = 1.0;
		...
		int      prevNoA = -1, prevNoB = -1;
		Vector3r searchDirA(0, 0, 0);
		int      newNoA = -1, newNoB = -1;
		Vector3r searchDirB(0, 0, 0);
		...
		do {
			...
			v1.x() = (ptOnBoundary.dot(orthogonalDir));
			v1.y() = (ptOnBoundary.dot(orthogonalDir2));
			v1.z() = 0.0;
			v2.x() = ((ptOnBoundary + newSearchDir).dot(orthogonalDir));
			v2.y() = ((ptOnBoundary + newSearchDir).dot(orthogonalDir2));
			v2.z() = 0.0;
			v3.x() = (contactPoint.dot(orthogonalDir));
			v3.y() = (contactPoint.dot(orthogonalDir2));
			v3.z() = 0.0;
			...
			if (getPtOnParticleAreaNormal(
			            cm1, state1, Vector3r(0, 0, 0) /* shift2 */, ptOnBoundary, searchDir, prevNoA, ptOnP1a, searchDirA, newNoA)) {
				prevNoA = newNoA;
			} else {
			...
			if ((p1 - p2).norm() > pow(10, -6)) {
				...
				dirA = searchDirA - (searchDirA.dot(contactNormal)) * contactNormal;
				dirA.normalize();
				dirB = searchDirB - (searchDirB.dot(contactNormal)) * contactNormal;
				dirB.normalize();
			...
			if (count > 12) {
				if (fabs((newPt - secondPoint).norm() - distanceBackup) < pow(10, -7)) {
					break;
				}
			}
			//#endif
			...
			count++;
			if (count > 60) {
				std::cout << "area polygon count: " << count << ", distance: " << (newPt - firstPoint).norm()
				          << ", dist2nd: " << (newPt - secondPoint).norm() << ", prevNoA: " << prevNoA << ", prevNoB: " << prevNoB << endl;
				areaTri = 0.0;
				tol     = 100.0 * tol;
			}
...
/* ***************************************************************************************************************************** */
bool Ig2_PB_PB_ScGeom::getPtOnParticleAreaNormal(
        const shared_ptr<Shape>& cm1,
        const State&             state1,
        const Vector3r&          shift2,
	...
	for (int i = 0; i < planeNo; i++) {
		...
		if (Mathr::Sign(dotProd) > 0.0 && fabs(dotProd) > pow(10.0, -3)) { //planeNormal and searchDirection converges
			...
			if (fabs(planeNormal.dot(newPoint) - d) > pow(10, -5)) {
				std::cout << "distance to plane: " << fabs(planeNormal.dot(newPoint) - d) << ", planeNormal: " << planeNormal
				          << ", prevDir: " << prevDir << ", u: " << u << ", previousPt: " << previousPt << ", d: " << d
				          << ", (planeNormal.dot(-prevDir)): " << (planeNormal.dot(-prevDir))
				          << ", (planeNormal.dot(previousPt) - d): " << (planeNormal.dot(previousPt) - d) << endl;
			}
...
/* ***************************************************************************************************************************** */
bool Ig2_PB_PB_ScGeom::customSolveAnalyticCentre(
        const shared_ptr<Shape>& cm1, const State& state1, const shared_ptr<Shape>& cm2, const State& state2, const Vector3r& shift2, Vector3r& contactPt)
{
	bool            converge    = true;
	PotentialBlock* s1          = static_cast<PotentialBlock*>(cm1.get());
	PotentialBlock* s2          = static_cast<PotentialBlock*>(cm2.get());
	...
	dgemm_(&blasNT,
	       ...
	       &blas0,
	       &blasP1Q[0],
	       &blas3); /*Matrix mulitplication Q1*P1 */
	dgemm_(&blasNT,
	       &blasNT,
	       &blas3,
	       &planeNoB,
	       ...
	       &blas0,
	       &blasP2Q[0],
	       &blas3); /*Matrix mulitplication Q2*P2 */

	for (int k = 0; k < planeNoA; k++) {
		d1[k] = -1.0
		        * (blasP1Q[3 * k] * (contactPt.x() - state1.pos.x()) + blasP1Q[3 * k + 1] * (contactPt.y() - state1.pos.y())
		           + blasP1Q[3 * k + 2] * (contactPt.z() - state1.pos.z()) - d1[k]);
	}
	for (int k = 0; k < planeNoB; k++) {
		d2[k] = -1.0
		        * (blasP2Q[3 * k] * (contactPt.x() - state2.pos.x() - shift2[0]) + blasP2Q[3 * k + 1] * (contactPt.y() - state2.pos.y() - shift2[1])
		           + blasP2Q[3 * k + 2] * (contactPt.z() - state2.pos.z() - shift2[2]) - d2[k]);
	}
	...
	double B[totalPlanes];
	double D[totalPlanes];
	double Dinvert[totalPlanes];
	double Ddiag[totalPlanes * totalPlanes];
	double oriMinD = 9999999;
	...
	while (iter < 50) {
		...
		char   UPLO = 'L';
		int    KD   = varNo - 1;
		int    nrhs = 1; //FIXME: varNo, UPLO, KD, nrhs could be defined as "const" variables
...
//#endif
...
#endif
		if (info != 0) {
			std::cout << "linear algebra error, iter:" << iter << endl;
			converge = false;
			break;
		}
		...
		while (Mathr::Sign(minD) * 1.0 < 0.0) {
			...
			if (backtrack < pow(10, -15)) {
				std::cout << "backtrack: " << backtrack << ", iter: " << iter << ", blasStep: " << blasStep[0] << "," << blasStep[1] << ","
				          << blasStep[2] << ", minD: " << minD << ", oriMinD: " << oriMinD << ", grad: " << grad[0] << "," << grad[1] << ","
				          << grad[2] << ", xx: " << xx[0] << "," << xx[1] << "," << xx[2] << ", Hess: " << Hess[0] << "," << Hess[1] << ","
				          << Hess[2] << "," << Hess[3] << "," << Hess[4] << "," << Hess[5] << "," << Hess[6] << "," << Hess[7] << "," << Hess[8]
				          << endl;
				converge = false;
				break;
			}
		...
		while (val > orival + backtrack * 0.01 * blasFprime) {
			...
			if (backtrack < pow(10, -15)) {
				std::cout << "initbacktrack: " << initbacktrack << ", backtrack: " << backtrack << ", iter: " << iter
				          << ", blasStep: " << blasStep[0] << "," << blasStep[1] << "," << blasStep[2] << ", blasFprime: " << blasFprime
				          << ", val: " << val << ", orival: " << orival << ", oriMinD: " << oriMinD << ", minD: " << minD << endl;
...
/* ***************************************************************************************************************************** */
bool Ig2_PB_PB_ScGeom::startingPointFeasibilityCLP(
        const shared_ptr<Shape>& cm1,
        const State&             state1,
        const shared_ptr<Shape>& cm2,
        const State&             state2,
        ...
        Vector3r&                contactPoint /*, bool &convergeFeasibility*/)
{
	...
	Vector3r xGlobal(0, 0, 0);
	double   rescale = 1.0; //bool converge = true;
	/* minimise s */
	/* s.t. Ax - s <= d*/
	PotentialBlock* s1 = static_cast<PotentialBlock*>(cm1.get());
	PotentialBlock* s2 = static_cast<PotentialBlock*>(cm2.get());
	...
	for (int i = 0; i < planeNoA; i++) {
		A1(i, 0) = s1->a[i] / rescale;
		A1(i, 1) = s1->b[i] / rescale;
		A1(i, 2) = s1->c[i] / rescale;
	}
	Eigen::MatrixXd A2 = Eigen::MatrixXd::Zero(planeNoB, 3);
	for (int i = 0; i < planeNoB; i++) {
		A2(i, 0) = s2->a[i] / rescale;
		A2(i, 1) = s2->b[i] / rescale;
		A2(i, 2) = s2->c[i] / rescale;
	}
...
Vector3r Ig2_PB_PB_ScGeom::getNormal(const shared_ptr<Shape>& cm1, const State& state1, const Vector3r& shift2, const Vector3r newTrial, const bool twoD)
{
	...
	bool           isSphere        = true;
	int            minNo           = 0;
	double         closestDistance = pow(10, 12);
	for (int i = 0; i < planeNo; i++) {
		plane = s1->a[i] * x + s1->b[i] * y + s1->c[i] * z - s1->d[i];
		if (plane < pow(10, -15)) {
...
/* ***************************************************************************************************************************** */
bool Ig2_PB_PB_ScGeom::customSolve(
        const shared_ptr<Shape>& cm1,
        const State&             state1,
        const shared_ptr<Shape>& cm2,
        const State&             state2,
        ...
        bool                     warmstart)
{
	...
	int      planeNoB3 = 3 + planeNoB; //int planeNoA2 =planeNoA*planeNoA; int planeNoB2=planeNoB*planeNoB;
	...
	char                       transA    = 'N'; /* char transB = 'N'; int  blasM = 3; int  blasN = 3; */
	int                        blasK     = 3;
	/* int blasLDA = 3; */ int blasLDB   = 3;
	double                     blasAlpha = 1.0;
	double                     blasBeta  = 0.0;
	/* int blasLDC = 3; */ int incx      = 1;
	int                        incy      = 1;
...
blasX[3]=s;
	...
	for( int j=1; j<=varNo ; j++){
		...
		if (info != 0) {
			std::cout << "chol error"
			          << ", planeA: " << planeNoA << ", planeB: " << planeNoB << endl;
			//return false;
			/* LU factorization */
			for (int i = 0; i < varNo; i++) {
		...
		while (s < 0.0 || u1 < 0.0 || u2 < 0.0 || minWL < 0.0) {
			...
			if (count == 200) {
				std::cout << "count: " << count << ", totalIter: " << totalIter << ", backtrack: " << backtrack << "s : " << s << ", u1: " << u1
				          << ", u2: " << u2 << ", wLmincoeff: " << minWL << endl;
		...
		while (newval > val + backtrack * 0.01 * blasFprime) {
			...
			if (backtrack < pow(10, -11)) {
				std::cout << "iter: " << iter << ", totalIter: " << totalIter << ", backtrack: " << backtrack << ", val: " << val
				          << ", newval: " << newval << ", t: " << t << ", fprime: " << blasFprime << endl;
				break;
			}
		}
