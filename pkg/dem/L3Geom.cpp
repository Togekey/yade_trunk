        ...
        Real                           r2)
{
	...
	if (!I->geom) {
		...
		Vector3r locY = normal.cross(std::abs(normal[1]) < std::abs(normal[2]) ? Vector3r::UnitY() : Vector3r::UnitZ());
        ...
        const shared_ptr<Interaction>& I)
{
	...
	if (!I->isReal() && std::abs(dist) > radius && !force) {
		return false;
        ...
        const shared_ptr<Interaction>& I)
{
	...
	if (std::abs(planeDist) > radius && !I->isReal() && !force)
