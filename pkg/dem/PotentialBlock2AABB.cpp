...
//			Vector3r vertex = r*(pp->oriAabb.conjugate()*pp->vertices[i]);			
			Vector3r vertex = se3.orientation*pp->vertices[i];

			aabbMin=aabbMin.cwiseMin(vertex);
			aabbMax=aabbMax.cwiseMax(vertex);

//			aabbMin = Vector3r( min(aabbMin[0],vertex[0]), min(aabbMin[1],vertex[1]), min(aabbMin[2],vertex[2]) );
//			aabbMax = Vector3r( max(aabbMax[0],vertex[0]), max(aabbMax[1],vertex[1]), max(aabbMax[2],vertex[2]) );
...
//		pp->halfSize = 1.0*Vector3r(std::max(halfSizeMin.x(),halfSizeMax.x()),std::max(halfSizeMin.y(),halfSizeMax.y()),std::max(halfSizeMin.z(),halfSizeMax.z()));
		return;
	}
