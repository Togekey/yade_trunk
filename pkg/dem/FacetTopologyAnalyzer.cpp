...
void FacetTopologyAnalyzer::action()
{
	...
	for (size_t i = 0; i < nVertices; i++) {
		...
		while (++j < nVertices && (vv[j]->coord - vv[i]->coord) <= tolerance) {
			...
			if (std::abs(vi->pos[0] - vj->pos[0]) <= tolerance && std::abs(vi->pos[1] - vj->pos[1]) <= tolerance
			    && std::abs(vi->pos[2] - vj->pos[2]) <= tolerance && (vi->pos - vj->pos).squaredNorm() <= tolerance * tolerance) {
				...
				if (vi->index < vj->index) {
					vi->nextIdentical.push_back(vj);
					vj->isLowestIndex = false;
				} else {
					vj->nextIdentical.push_back(vi);
					vi->isLowestIndex = false;
				}
			}
		}
	}
	...
	for (size_t i = 0; i < nTopo; i++) {
		...
		while (++j < nTopo) {
					...
					if (edge[k] == 2) {
						LOG_FATAL(
						        "No edge identified for 2 vertices " << vvv[0] << "," << vvv[1] << " (facet #" << tt->id
						                                             << " is: " << tt->vertices[0] << "," << tt->vertices[1] << ","
						                                             << tt->vertices[2] << ")");
			...
			f1->edgeAdjIds[ei] = ti->id;
			f2->edgeAdjIds[ej] = tj->id;
			...
			//TRVAR2(n1g,n2g);
			Vector3r contEdge1g
			        = b1->state->ori * (f1->vertices[(ei + 1) % 3] - f1->vertices[ei]); // vector of the edge of contact in global coords
			Quaternionr q12;
			q12.setFromTwoVectors(n1g, (invNormals ? -1. : 1.) * n2g);
			AngleAxisr aa12(q12);
			...
			commonEdgesFound++;
			LOG_TRACE(
			        "Found adjacent #" << ti->id << "+#" << tj->id << "; common edges " << ei << "+" << ej << ", normals "
