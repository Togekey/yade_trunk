...
//#endif
		...
		phys->kn          = normalStiffness * std::max(pow(10, -15), phys->contactArea);
...
//shearForce -= phys->ksVol*shearIncrement*std::max(pow(10,-15),phys->contactArea);
		...
		phys->ks = Ks * std::max(pow(10, -15), phys->contactArea);
...
// I compacted the existing commented code block below into the statement above, in order to minimise the number of "if" statements. I keep the old code for now, just in case a bug occurs @vsangelidakis
	...
	Real Cn_crit = 2. * sqrt(mbar * phys->kn); // Critical damping coefficient (normal direction) 2.*sqrt(mbar*std::min(phys->kn,phys->ks))
...
double Law2_SCG_KnKsPBPhys_KnKsPBLaw::stressUpdateVec(
        shared_ptr<IPhys>& /*ip*/,
        const Vector3r Fs_prev /*prev shear force*/,
        const Vector3r du /*shear displacement increment*/,
        const double   beta_prev /* prev plastic displacements*/,
        const double   Ks /*shear stiffness */,
        const double   fN /*normal force*/,
        const double   phi_b /*peak friction angle*/,
        Vector3r&      newFs /*new shear force*/)
{
	...
	if (newFs.norm() - maxFs > pow(10, -11) && fN > pow(10, -11) && (Ks * du).norm() > pow(10, -11)) {
		...
		double upperBound = du.norm() / Fs_prev.norm();
		if (std::isnan(upperBound) == true) {
			upperBound = 1.0;
		}
		if (std::isinf(upperBound) == true || upperBound > pow(10.0, 12)) {
...
/* ***************************************************************************************************************************** */
/** FUNCTION RETURNS PLASTIC MULTIPLIER RATE (beta) AND CURRENT SHEAR FORCE */
double Law2_SCG_KnKsPBPhys_KnKsPBLaw::stressUpdateVecTalesnick(
        shared_ptr<IPhys>& /*ip*/,
        const Vector3r Fs_prev /*prev shear force*/,
        const Vector3r du /*shear displacement increment*/,
        const double   beta_prev /* prev plastic displacements*/,
