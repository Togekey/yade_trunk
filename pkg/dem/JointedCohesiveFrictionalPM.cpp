...
bool Law2_ScGeom_JCFpmPhys_JointedCohesiveFrictionalPM::go(shared_ptr<IGeom>& ig, shared_ptr<IPhys>& ip, Interaction* contact)
{
	...
	if (!phys->momentBroken && useStrainEnergy)
		phys->strainEnergy = 0.5 * ((pow(phys->normalForce.norm(), 2) / phys->kn) + (pow(phys->shearForce.norm(), 2) / phys->ks));
	else if (!phys->momentBroken && !useStrainEnergy)
		computeKineticEnergy(phys, b1, b2);
	...
	if (phys->momentBroken && recordMoments && !phys->momentCalculated) {
		...
		if (phys->momentCalculated && phys->momentMagnitude != 0) {
			...
			file << boost::lexical_cast<string>(scene->iter) << " " << boost::lexical_cast<string>(phys->momentCentroid[0]) << " "
			     ...
			     << boost::lexical_cast<string>(phys->eventNumber) << " " << boost::lexical_cast<string>(scene->time) << " "
			     << boost::lexical_cast<string>(phys->eventBeginTime) << endl;
		...
		if (phys->isCohesive && (phys->FnMax > 0) && (std::abs(D) > Dtensile)) {
			...
			if (recordCracks) {
				...
				if ((smoothJoint) && (phys->isOnJoint)) {
					...
					crackNormal = geom->normal;
				}
				file << boost::lexical_cast<string>(scene->iter) << " " << boost::lexical_cast<string>(scene->time) << " "
				     << boost::lexical_cast<string>(geom->contactPoint[0]) << " " << boost::lexical_cast<string>(geom->contactPoint[1]) << " "
				     << boost::lexical_cast<string>(geom->contactPoint[2]) << " " << 1 << " "
				     << boost::lexical_cast<string>(0.5 * (geom->radius1 + geom->radius2)) << " " << boost::lexical_cast<string>(crackNormal[0])
				     << " " << boost::lexical_cast<string>(crackNormal[1]) << " " << boost::lexical_cast<string>(crackNormal[2]) << " "
				     << boost::lexical_cast<string>(0.5 * (((scalarNF * scalarNF) / phys->kn) + ((scalarSF * scalarSF) / phys->ks))) << " "
				     << boost::lexical_cast<string>(phys->isOnJoint) << endl;
			}
	...
	/* Apply forces */
	if ((smoothJoint) && (phys->isOnJoint)) {
		phys->normalForce = Fn * phys->jointNormal;
	} else {
...
void Law2_ScGeom_JCFpmPhys_JointedCohesiveFrictionalPM::computeKineticEnergy(JCFpmPhys* phys, Body* b1, Body* b2)
{
	...
	Real kineticEnergy1 = 0.5
	        * (m1 * (vel1[0] * vel1[0] + vel1[1] * vel1[1] + vel1[2] * vel1[2])
	           + inert1[0] * (angVel1[0] * angVel1[0] + angVel1[1] * angVel1[1] + angVel1[2] * angVel1[2]));
	Real kineticEnergy2 = 0.5
	        * (m2 * (vel2[0] * vel2[0] + vel2[1] * vel2[1] + vel2[2] * vel2[2])
...
void Law2_ScGeom_JCFpmPhys_JointedCohesiveFrictionalPM::addUniqueIntsToList(JCFpmPhys* phys, JCFpmPhys* nearbyPhys)
{
	...
	for (unsigned int i = 0; i < nearbyPhys->nearbyInts.size(); i++) {
		...
		if (pushBack && nearbyPhys->nearbyInts[i]) {
			...
			phys->momentEnergy
			        += (useStrainEnergy
			                    ? nrgPhys->strainEnergy
			                    : nrgPhys->kineticEnergy); // while we are here we update the reference strain (or kinetic) energy by adding the strain (or kinetic) energy of the newly added ints
		}
	}
}
...
void Law2_ScGeom_JCFpmPhys_JointedCohesiveFrictionalPM::clusterInteractions(JCFpmPhys* phys, Interaction* contact)
{
	...
	originalPhys->clusterInts.push_back(
	        scene->interactions->find(contact->getId1(), contact->getId2())); // add this broken interaction to list of broken bonds in this event
}
...
void Ip2_JCFpmMat_JCFpmMat_JCFpmPhys::go(const shared_ptr<Material>& b1, const shared_ptr<Material>& b2, const shared_ptr<Interaction>& interaction)
{
	...
	if (((cohesiveTresholdIteration < 0) || (scene->iter < cohesiveTresholdIteration)) && (std::min(SigT1, SigT2) > 0 || std::min(Coh1, Coh2) > 0)
	    && (yade1->type == yade2->type)) {
	...
	contactPhysics->isCohesive ? contactPhysics->tanFrictionAngle = std::tan(std::min(f1, f2))
	                           : contactPhysics->tanFrictionAngle = std::tan(std::min(rf1, rf2));
	...
	if (st1->onJoint && st2->onJoint) {
		if ((((st1->jointNormal1.cross(st2->jointNormal1)).norm() < 0.1) && (st1->jointNormal1.dot(st2->jointNormal1) < 0))
			...
			contactPhysics->jointNormal = st1->jointNormal3;
		        ...
		        && ((((st1->jointNormal1.cross(st2->jointNormal1)).norm() > 0.1) && ((st1->jointNormal1.cross(st2->jointNormal2)).norm() > 0.1)
		             && ((st1->jointNormal1.cross(st2->jointNormal3)).norm() > 0.1))
		            || (((st1->jointNormal2.cross(st2->jointNormal1)).norm() > 0.1) && ((st1->jointNormal2.cross(st2->jointNormal2)).norm() > 0.1)
	...
	if (contactPhysics->isOnJoint) {
		...
		///to set if the contact is cohesive or not
		if (((cohesiveTresholdIteration < 0) || (scene->iter < cohesiveTresholdIteration))
		    && (std::min(jcoh1, jcoh2) > 0 || std::min(jSigT1, jSigT2) > 0)) {
			contactPhysics->isCohesive = true;
			st1->nbInitBonds++;
			st2->nbInitBonds++;
		} else {
