...
void RockLiningGlobal::action()
{
	...
	if (openingCreated == true && installed == false) {
		...
		for (int n = 0; n < totalNodes; n++) {
			...
			for (int i = 0; i < totalPlanes; i++) {
				...
				double   planeD = plane.dot(state1->pos) + pb->d[i] + pb->r;
	...
	if (installed == true && blockIDs.size() >= 2) {
		...
		for (int j = 0; j < blockNo; j++) {
			...
			double shearForcesB
			        = -12.0 * EI / (pow(lengthNode[j], 3)) * (localYa - localYb) + 6.0 * EI / (pow(lengthNode[j], 2)) * (thetaA + thetaB);
...
bool RockLiningGlobal::installLining(
        const PotentialBlock* s1, const State* state1, const Vector3r startingPt, const Vector3r direction, const double length, Vector3r& intersectionPt)
{
