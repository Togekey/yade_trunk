...
YADE_PLUGIN(/* self-contained in hpp: */ (Ig2_Polyhedra_Polyhedra_PolyhedraGeom)(Ig2_Wall_Polyhedra_PolyhedraGeom)(Ig2_Facet_Polyhedra_PolyhedraGeom)(
        Ig2_Sphere_Polyhedra_ScGeom)(Ig2_Polyhedra_Polyhedra_ScGeom)(Ig2_Polyhedra_Polyhedra_PolyhedraGeomOrScGeom));
        ...
        const shared_ptr<Interaction>& interaction)
{
	...
	Polyhedron Int;
	Int = Polyhedron_Polyhedron_intersection(
	        PA, PB, ToCGALPoint(bang->contactPoint), ToCGALPoint(se31.position), ToCGALPoint(se32.position + shift2), bang->sep_plane);
	...
	if ((!Is_inside_Polyhedron(PA, ToCGALPoint(centroid))) or (!Is_inside_Polyhedron(PB, ToCGALPoint(centroid)))) {
		bang->equivalentPenetrationDepth = 0;
		return !isNew;
	}
...
bool Ig2_Wall_Polyhedra_PolyhedraGeom::go(
        const shared_ptr<Shape>& shape1,
        const shared_ptr<Shape>& shape2,
        const State&             state1,
        const State&             state2,
        ...
        const shared_ptr<Interaction>& interaction)
{
	...
	Vector3r centroid;
	P_volume_centroid(Int, &volume, &centroid);
	if (std::isnan(volume) || volume <= 1E-25 || volume > B->GetVolume()) {
		bang->equivalentPenetrationDepth = 0;
		return true;
	}
	if (!Is_inside_Polyhedron(PB, ToCGALPoint(centroid))) {
		bang->equivalentPenetrationDepth = 0;
		return true;
	}
...
bool Ig2_Facet_Polyhedra_PolyhedraGeom::go(
        const shared_ptr<Shape>& shape1,
        const shared_ptr<Shape>& shape2,
        const State&             state1,
        const State&             state2,
        ...
        const shared_ptr<Interaction>& interaction)
{
	...
	v.resize(6);
	for (int i = 0; i < 3; i++)
		v[i] = ToCGALPoint(se31.orientation * A->vertices[i] + se31.position); // vertices in global coordinates
	...
	for (int i = 3; i < 6; i++)
		v[i] = v[i - 3] - f_normal / f_area * 0.05 * sqrt(f_area); // vertices in global coordinates
	...
	Polyhedron Int;
	Int = Polyhedron_Polyhedron_intersection(
	        PA, PB, ToCGALPoint(bang->contactPoint), ToCGALPoint(se31.position), ToCGALPoint(se32.position), bang->sep_plane);
...
bool Ig2_Sphere_Polyhedra_ScGeom::go(
        const shared_ptr<Shape>& shape1,
        const shared_ptr<Shape>& shape2,
        const State&             state1,
        const State&             state2,
        ...
        const shared_ptr<Interaction>& interaction)
{
	...
	for (unsigned int i = 0; i < pts.size(); i++)
		pts[i] = se32.position + se32.orientation * pts[i];
...
//**********************************************************************************
/*! Plyhedra -> ScGeom. */
bool Ig2_Polyhedra_Polyhedra_ScGeom::go(
        const shared_ptr<Shape>&       shape1,
        const shared_ptr<Shape>&       shape2,
        const State&                   state1,
        const State&                   state2,
...
bool Ig2_Polyhedra_Polyhedra_ScGeom::goReverse(
        const shared_ptr<Shape>&       shape1,
        const shared_ptr<Shape>&       shape2,
        const State&                   state1,
        const State&                   state2,
...
bool Ig2_Polyhedra_Polyhedra_PolyhedraGeomOrScGeom::go(
        const shared_ptr<Shape>&       shape1,
        const shared_ptr<Shape>&       shape2,
        const State&                   state1,
        const State&                   state2,
...
bool Ig2_Polyhedra_Polyhedra_PolyhedraGeomOrScGeom::goReverse(
        const shared_ptr<Shape>&       shape1,
        const shared_ptr<Shape>&       shape2,
        const State&                   state1,
        const State&                   state2,
