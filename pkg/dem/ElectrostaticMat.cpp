...
void Ip2_FrictMat_FrictMat_ElectrostaticPhys::go(
...
Real Law2_ScGeom_ElectrostaticPhys::normalForce_DL_Adim(ElectrostaticPhys* phys, ScGeom* geom, Real const& undot, bool isNew)
{
	...
	if (phys->nun <= 0.) {
		if (!warnedOnce)
			LOG_WARN("Can't solve with dimentionless-exponential method without fluid!");
		warnedOnce = true;
		phys->u    = -1;
		return -1;
	}
	...
	if (isNew) {
		phys->u     = -geom->penetrationDepth - undot * scene->dt;
		phys->delta = std::log(phys->u / a);
	}

	Real d = DL_DichoAdimExp_integrate_u(
	        ...
	        a / phys->DebyeLength,
	        phys->prevDotU,
	        scene->dt * a * phys->kn / phys->nun,
	        phys->delta,
	        phys->nun / phys->kn / std::pow(a, 2) * undot); // Solution by dichotomy

	phys->normalForce = phys->kn * (-geom->penetrationDepth - a * std::exp(d)) * geom->normal;
	phys->normalContactForce
	        = (phys->nun > 0.) ? Vector3r(-phys->kn * (std::max(2. * a * phys->eps - a * std::exp(d), 0.)) * geom->normal) : phys->normalForce;
...
Real Law2_ScGeom_ElectrostaticPhys::DL_DichoAdimExp_integrate_u(
        Real const& un, Real const& eps, Real const& alpha, Real const& Z, Real const& K, Real& prevDotU, Real const& dt, Real const& prev_d, Real const& undot)
{
	Real F = 0.;
...
Real Law2_ScGeom_ElectrostaticPhys::ObjF(
        Real const& un,
        Real const& eps,
        Real const& alpha,
        Real const& prevDotU,
        ...
        Real const& d)
{
	...
	return theta * (un - std::exp(d) * (1. + a) + a * eps + K * Z * std::exp(-K * std::exp(d))) + (1. - theta) * prevDotU * std::exp(prev_d - d)
	        - 1. / dt * (1. - std::exp(prev_d - d));
...
bool Law2_ScGeom_ElectrostaticPhys::go(shared_ptr<IGeom>& iGeom, shared_ptr<IPhys>& iPhys, Interaction* interaction)
{
	...
	if (-geom->penetrationDepth > a && -geom->penetrationDepth > 10. * phys->DebyeLength) {
		return false;
	}
