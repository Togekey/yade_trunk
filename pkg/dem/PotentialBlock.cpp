...
void PotentialBlock::postLoad(PotentialBlock&)
{
	if (vertices.empty() and (not a.empty())) { // i.e. if the particle is not initialised
		...
		if (std::abs(centr.x()) / R < 1e-6) {
			centr.x() = 0;
		}
		if (std::abs(centr.y()) / R < 1e-6) {
			centr.y() = 0;
		}
		if (std::abs(centr.z()) / R < 1e-6) {
			centr.z() = 0;
		}
		...
		if (std::abs(Ixy) + std::abs(Iyz) + std::abs(Iyz) < 1e-15) {
			...
			if (fabs(Izz) < pow(10, -15)) {
				Izz = 0.0;
			} // Or I could normalise the checked inertia values to make them dimensionless, by dividing with pow(R,5) or V*pow(R,3) or something similar
