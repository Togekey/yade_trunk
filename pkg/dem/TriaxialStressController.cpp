...
void TriaxialStressController::controlExternalStress(
        int wall, Vector3r resultantForce, State* p, Real wall_max_vel) // controls walls such that Sum Forces from Sample on Wall = resultantForce
{
	scene->forces.sync();
	...
	if (translation != 0) {
		if (stiffness[wall] != 0) {
			...
			translation = wall_max_vel * Mathr::Sign(translation) * scene->dt;
	}
	previousTranslation[wall]
	        = (1 - stressDamping) * translation * normal[wall] + 0.8 * previousTranslation[wall]; // formula for "steady-flow" evolution with fluctuations
...
void TriaxialStressController::action()
{
	...
	} else //if internal compaction
	{
		...
		if (isARadiusControlIteration) {
			...
			if (std::abs(sigma_iso_) <= std::abs(meanStress))
			...
			if (meanStress == 0)
				...
				if (sigma_iso_
				    < 0) // compressive case: we have to increase radii if meanStress > sigma_iso_, considering that sigma_iso_ < 0. We end with the same expression as before sign change
					previousMultiplier
					        = 1. + (sigma_iso_ - meanStress) / sigma_iso_ * (maxMultiplier - 1.); // = (Dsigma/apparentModulus)*0.7
				else // tensile case: we have to increase radii if meanStress > sigma_iso_ too. But here sigma_iso_ > 0 => another expression
					previousMultiplier
					        = 1. + (meanStress - sigma_iso_) / sigma_iso_ * (maxMultiplier - 1.); // = (Dsigma/apparentModulus)*0.7
			}
