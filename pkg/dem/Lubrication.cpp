...
Real Law2_ScGeom_ImplicitLubricationPhys::normalForce_AdimExp(LubricationPhys* phys, ScGeom* geom, Real undot, bool isNew, bool dichotomie)
{
	...
	if (phys->nun <= 0.) {
		if (!warnedOnce)
			LOG_WARN("Can't solve with dimentionless-exponential method without fluid! using exact.");
		warnedOnce = true;
	...
	if (dichotomie)
		d = DichoAdimExp_integrate_u(
		        ...
		        phys->delta,
		        (3. / 2. * phys->nun) / phys->kn / std::pow(a, 2) * undot,
		        phys->contact ? phys->ladh : 0.);
	else
		d = NRAdimExp_integrate_u(
		        ...
		        scene->dt * a * phys->kn / (phys->nun * 3. / 2.),
		        phys->delta,
		        (3. / 2. * phys->nun) / phys->kn / std::pow(a, 2) * undot); // Newton-Rafson
        ...
        Real const& un, Real const& eps, Real const& alpha, Real& prevDotU, Real const& dt, Real const& prev_d, Real const& undot, int depth)
{
	...
	for (i = 0; i < MaxIter; i++) {
		...
		Real ratio
		        = (dt * (theta * (-(1. + a) * std::exp(d) + a * eps + un) + (1. - theta) * std::exp(prev_d - d) * prevDotU) - 1. + std::exp(prev_d - d))
		        / (dt * theta * (-2. * (1. + a) * std::exp(d) + a * eps + un) - 1.);

		F = theta * std::exp(d) * (-(1. + a) * std::exp(d) + a * eps + un) + (1. - theta) * std::exp(prev_d) * prevDotU
		        - 1. / dt * (std::exp(d) - std::exp(prev_d));
        ...
        Real const& un, Real const& eps, Real const& alpha, Real& prevDotU, Real const& dt, Real const& prev_d, Real const& undot, Real const& ladh)
{
	...
	Real F_left(ObjF(un, eps, alpha, prevDotU, dt, prev_d, undot, d_left, ladh));
	Real F_right(ObjF(un, eps, alpha, prevDotU, dt, prev_d, undot, d_right, ladh));
	Real d;
	...
	while (F_left * F_right >= 0. && std::isfinite(F_left) && std::isfinite(F_right)) {
		d_left += inc;
		d_right += inc;
		F_left  = ObjF(un, eps, alpha, prevDotU, dt, prev_d, undot, d_left, ladh);
		F_right = ObjF(un, eps, alpha, prevDotU, dt, prev_d, undot, d_right, ladh);
	}

	if ((!std::isfinite(F_left) || !std::isfinite(F_right))) {
		if (debug)
			LOG_WARN("Wrong direction");
		inc     = -inc; // RE-INIT
		d_left  = prev_d - 1.;
		d_right = prev_d + 1.;
		while (F_left * F_right >= 0. && std::isfinite(F_left) && std::isfinite(F_right)) {
	...
	for (i = 0; i < MaxIter; i++) {
		...
		d = (d_left + d_right) / 2.;
		F = ObjF(un, eps, alpha, prevDotU, dt, prev_d, undot, d, ladh);
	...
	Real a   = (std::exp(d) < eps + ladh) ? alpha : 0.;
	prevDotU = -(1. + a) * std::exp(d) + a * eps + un;
...
template <typename T> int sign(T val) { return (int)(T(0) < val) - (val < T(0)); }
...
Real Law2_ScGeom_ImplicitLubricationPhys::normalForce_trpz_adim(LubricationPhys* phys, ScGeom* geom, Real undot, bool isNew)
{
	...
	if (phys->nun <= 0.) {
		if (!warnedOnce)
			LOG_WARN("Can't solve with dimentionless-exponential method without fluid! using dimentional exact.");
		warnedOnce = true;
		return normalForce_trapezoidal(phys, geom, undot, isNew);
	}
...
Real Law2_ScGeom_ImplicitLubricationPhys::trapz_integrate_u_adim(
        Real const& u_n, Real const& eps, Real const& dt, Real const& prev_u, Real const& ladh, bool const& inContact, Real& prevDotU)
{
        ...
        int         depth)
{
	...
	if (delta < 0 or rr[0] < 0) { // recursive calls after halving the time increment if no positive solution found (no need to check r[1], always smaller)
		if (depth < maxSubSteps) { //sub-stepping
			...
			if (!warnedOnce) {
				LOG_WARN(
				        "minimal sub-step reached (depth=" << maxSubSteps
				                                           << "), the result may be innacurate. Increase maxSubSteps?"); /*warnedOnce=true;*/
			}
...
void Law2_ScGeom_ImplicitLubricationPhys::shearForce_firstOrder_log(LubricationPhys* phys, ScGeom* geom)
{
	...
	const Vector3r& dus = geom->shearIncrement();
	Real            kt  = phys->ks;
	Real nut = (phys->eta > 0.) ? M_PI * phys->eta / 2. * a * (-2. + (2. + std::exp(phys->delta)) * (std::log(2. + std::exp(phys->delta)) - phys->delta))
	                            : 0.;
	...
	if (phys->eta <= 0. || phys->u > 0.) {
		...
		if (a > phys->u) {
			if (activateRollLubrication && phys->eta > 0.)
				Cr = phys->nun * (3. / 2. * a + 63. / 500. * phys->u) * std::log(a / phys->u) * relRollVelocity;
			if (activateTwistLubrication && phys->eta > 0.)
				Ct = phys->nun * phys->u * std::log(a / phys->u) * relTwistVelocity;
		}
		// total torque
...
{
	Real a((geom->radius1 + geom->radius2) / 2.);
	if (resolution == 1 && debug && !warnedOnce) {
		LOG_DEBUG("This method use log(u/a) for shear and torque component calculation. Make sure phys->delta is set before calling this method.");
		warnedOnce = true;
	}
	...
	if (phys->delta > 0.) {
		if (activateRollLubrication && phys->eta > 0.)
			Cr = -phys->nun * a * 3. / 2. * (21. / 250. * std::exp(phys->delta) + 1.) * phys->delta * relRollVelocity;
		if (activateTwistLubrication && phys->eta > 0.)
			Ct = -phys->nun * a * std::exp(phys->delta) * phys->delta * relTwistVelocity;
	}
	// total torque
	C1 = -(geom->radius1 - geom->penetrationDepth / 2.) * phys->shearForce.cross(geom->normal) + Cr + Ct;
	C2 = -(geom->radius2 - geom->penetrationDepth / 2.) * phys->shearForce.cross(geom->normal) - Cr - Ct;
...
void Law2_ScGeom_ImplicitLubricationPhys::getStressForEachBody(
        vector<Matrix3r>& NCStresses, vector<Matrix3r>& SCStresses, vector<Matrix3r>& NLStresses, vector<Matrix3r>& SLStresses)
{
