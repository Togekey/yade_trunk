...
void UniaxialStrainer::init()
{
	...
	LOG_DEBUG(
	        "Reference particles: positive #" << posIds[0] << " at " << axisCoord(posIds[0]) << "; negative #" << negIds[0] << " at "
	                                          << axisCoord(negIds[0]));
	LOG_INFO("Setting initial length to " << originalLength << " (between #" << negIds[0] << " and #" << posIds[0] << ")");
	if (originalLength <= 0)
		throw runtime_error(
		        ("UniaxialStrainer: Initial length is negative or zero (swapped reference particles?)! " + boost::lexical_cast<string>(originalLength))
		                .c_str());
	...
	if (!std::isnan(std::numeric_limits<Real>::quiet_NaN())) {
		throw runtime_error("UniaxialStrainer: NaN's are not properly supported (compiled with -ffast-math?), which is required.");
	}

	if (std::isnan(strainRate)) {
		strainRate = absSpeed / originalLength;
		LOG_INFO("Computed new strainRate " << strainRate);
	} else {
		absSpeed = strainRate * originalLength;
	}

	if (!setSpeeds) {
		initAccelTime_s = initAccelTime >= 0 ? initAccelTime : scene->dt * (-initAccelTime);
		LOG_INFO(
		        "Strain speed will be " << absSpeed << ", strain rate " << strainRate << ", will be reached after " << initAccelTime_s << "s ("
		                                << initAccelTime_s / scene->dt << " steps).");
	} else {
		...
		switch (asymmetry) {
			...
			default:
				throw std::invalid_argument(
				        ("UniaxialStrainer: unknown asymmetry value " + boost::lexical_cast<string>(asymmetry) + " (should be -1,0,1)")
				                .c_str());
		}
		assert(p1 > p0);
...
void UniaxialStrainer::action()
{
	...
	if (!std::isnan(stopStrain)) {
		...
		if ((newStrain * stopStrain > 0)
		    && std::abs(newStrain) >= stopStrain) { // same sign of newStrain and stopStrain && over the limit from below in abs values
	...
	if (notYetReversed && limitStrain != 0 && ((currentStrainRate > 0 && strain > limitStrain) || (currentStrainRate < 0 && strain < limitStrain))) {
		...
		LOG_INFO("Reversed strain rate to " << currentStrainRate);
	}
