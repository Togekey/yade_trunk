...
void VTKRecorder::action()
{
	...
	if (recActive[REC_INTR]) {
		...
		FOREACH(const shared_ptr<Interaction>& I, *scene->interactions)
		{
			if (!I->isReal())
				continue;
			if (skipFacetIntr) {
				if (!(Body::byId(I->getId1())))
					continue;
				if (!(Body::byId(I->getId2())))
					continue;
				if (!(dynamic_cast<Sphere*>(Body::byId(I->getId1())->shape.get())))
					continue;
				if (!(dynamic_cast<Sphere*>(Body::byId(I->getId2())->shape.get())))
					continue;
			}
			...
			if (iterId2 == bIdVector.end() || iterId2 == bIdVector.end())
				continue;
	...
	if (recActive[REC_STRESS])
		Shop::getStressForEachBody(bodyStates);
	...
	FOREACH(const shared_ptr<Body>& b, *scene->bodies)
	{
		if (!b)
			continue;
		if (mask != 0 && !b->maskCompatible(mask))
			continue;
		if (recActive[REC_SPHERES]) {
			const Sphere* sphere = dynamic_cast<Sphere*>(b->shape.get());
			if (sphere) {
				if (skipNondynamic && b->state->blockedDOFs == State::DOF_ALL)
					continue;
				vtkIdType pid[1];
				...
				if (recActive[REC_LUBRICATION]) {
					...
					Real nc[9] = { (Real)ncs(0, 0), (Real)ncs(0, 1), (Real)ncs(0, 2), (Real)ncs(1, 0), (Real)ncs(1, 1),
						       (Real)ncs(1, 2), (Real)ncs(2, 0), (Real)ncs(2, 1), (Real)ncs(2, 2) };
					Real sc[9] = { (Real)scs(0, 0), (Real)scs(0, 1), (Real)scs(0, 2), (Real)scs(1, 0), (Real)scs(1, 1),
						       (Real)scs(1, 2), (Real)scs(2, 0), (Real)scs(2, 1), (Real)scs(2, 2) };
					Real nl[9] = { (Real)nls(0, 0), (Real)nls(0, 1), (Real)nls(0, 2), (Real)nls(1, 0), (Real)nls(1, 1),
						       (Real)nls(1, 2), (Real)nls(2, 0), (Real)nls(2, 1), (Real)nls(2, 2) };
					Real sl[9] = { (Real)sls(0, 0), (Real)sls(0, 1), (Real)sls(0, 2), (Real)sls(1, 0), (Real)sls(1, 1),
						       (Real)sls(1, 2), (Real)sls(2, 0), (Real)sls(2, 1), (Real)sls(2, 2) };
				...
				if (recActive[REC_CPM]) {
					...
					Real s[9] = { (Real)ss(0, 0), (Real)ss(0, 1), (Real)ss(0, 2), (Real)ss(1, 0), (Real)ss(1, 1),
						      (Real)ss(1, 2), (Real)ss(2, 0), (Real)ss(2, 1), (Real)ss(2, 2) };
	...
	if (compress)
		compressor = vtkSmartPointer<vtkZLibDataCompressor>::New();
	...
	if (recActive[REC_SPHERES]) {
		...
		if (recActive[REC_TEMP])
			spheresUg->GetPointData()->AddArray(spheresTemp);
		if (recActive[REC_CLUMPID])
