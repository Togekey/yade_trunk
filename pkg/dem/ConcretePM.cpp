...
{
	if (kappaD < epsCrackOnset || neverDamage)
		return 0;
	switch (damLaw) {
		case 0: // linear
...
/* constitutive law */
	...
	if (epsN > 0. && ((isCohesive && omega > omegaThreshold) || !isCohesive)) {
		...
		{
			boost::mutex::scoped_lock lock(st1->updateMutex);
			st1->numBrokenCohesive += 1; /* st1->epsPlBroken += epsPlSum; */
		}
		{
			boost::mutex::scoped_lock lock(st2->updateMutex);
        ...
        const shared_ptr<IPhys>& ip, const shared_ptr<Interaction>& i, const shared_ptr<Body>& /*b1*/, const shared_ptr<Body>& /*b2*/, bool /*wireFrame*/)
{
	...
	if (colorStrainRatio > 0)
		lineColor = Shop::scalarOnColorScale(phys->epsN / (phys->epsCrackOnset * colorStrainRatio));
	...
	if (contactLine)
		GLUtils::GLDrawLine(pos1, pos2, lineColor);
	if (dmgLabel) {
	...
	if (scene->isPeriodic) {
		cp = scene->cell->wrapShearedPt(cp);
	}
	if (epsT) {
