...
void KinemSimpleShearBox::computeAlpha()
{
	...
	alpha = Mathr::PI / 2.0
	        - angle; // right if the initial orientation of the body (on the beginning of the simulation) is q =(1,0,0,0) = FromAxisAngle((0,0,1),0)
}
...
{
	scene->forces.sync();
	Vector3r fSup = scene->forces.getForce(id_topbox);
