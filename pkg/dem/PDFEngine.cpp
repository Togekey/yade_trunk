...
vector<string> PDFSpheresDistanceCalculator::getDatas() const { return vector<string>({ std::to_string(m_h / m_N) }); }
...
vector<string> PDFSpheresVelocityCalculator::getDatas() const
{
	vector<string> ret;
	for (int i(0); i < 3; i++)
		ret.push_back(std::to_string(m_vel(i) / m_N));
	return ret;
}
...
vector<string> PDFSpheresIntrsCalculator::getDatas() const { return vector<string>({ std::to_string(m_P) }); }
