...
void PeriIsoCompressor::action()
{
	...
	if (charLen <= 0) {
		...
		if (!bv) {
			LOG_FATAL("No charLen defined and body #0 has no bound");
			throw;
		}
	...
	if (keepProportions) { // the same algo as below, but operating on quantitites averaged over all dimensions
		...
		if (std::abs(maxToAvg * avgGrow) > maxDisplPerStep)
		...
		for (int axis = 0; axis < 3; axis++) {
			...
			if (std::abs(cellGrow[axis]) > maxDisplPerStep)
			...
			if (std::abs((sigma[axis] - sigmaGoal) / sigmaGoal) > 5e-3)
				allStressesOK = false;
		}
	}
...
void PeriTriaxController::strainStressStiffUpdate()
{
	...
	FOREACH(const shared_ptr<Interaction>& I, *scene->interactions)
	{
		...
		if (!dynCell) {
			for (int i = 0; i < 3; i++)
				sumStiff[i] += std::abs(gsc->normal[i]) * nsi->kn + (1 - std::abs(gsc->normal[i])) * nsi->ks;
			n++;
		}
	}
...
void PeriTriaxController::action()
{
	...
	for (int axis = 0; axis < 3; axis++) {
		...
		if (std::abs(strain_rate) > maxStrainRate[axis])
		...
		if (stressMask & (1 << axis)) {
			...
			if ((goal[axis] != 0 && std::abs((curr - goal[axis]) / goal[axis]) > 1e-6) || std::abs(curr - goal[axis]) > 1e-6) {
				allOk = false;
				if (doUpdate)
					LOG_DEBUG("Strain not OK; " << std::abs(curr - goal[axis]) << ">1e-6");
			}
		}
	}
	...
	for (int k = 0; k < 3; k++)
		strainRate[k] = scene->cell->velGrad(k, k);
	...
	if (allOk) {
		if (doUpdate || currUnbalanced < 0) {
			currUnbalanced = Shop::unbalancedForce(/*useMaxForce=*/false, scene);
			LOG_DEBUG(
			        "Stress/strain=" << (stressMask & 1 ? stress[0] : strain[0]) << "," << (stressMask & 2 ? stress[1] : strain[1]) << ","
			                         << (stressMask & 4 ? stress[2] : strain[2]) << ", goal=" << goal << ", unbalanced=" << currUnbalanced);
		}
		if (currUnbalanced < maxUnbalanced) {
...
void Peri3dController::action()
{
	...
	*/
	...
	if (progress == 0) {
		...
		pathSizes[0] = xxPath.size();
		pathSizes[1] = yyPath.size();
		pathSizes[2] = zzPath.size();
		pathSizes[3] = yzPath.size();
		pathSizes[4] = zxPath.size();
		pathSizes[5] = xyPath.size();
		for (int i = 0; i < 6; i++) {
			pathsCounter[i] = 0;
		} // inidicator in which part of the path we are
...
#define PATH_OP_OP(pi, op1i, op2i)                                                                                                                             \
	(pi == 0 ? xxPath : pi == 1 ? yyPath : pi == 2 ? zzPath : pi == 3 ? yzPath : pi == 4 ? zxPath : xyPath)[op1i].operator()(op2i)
		...
		for (int i = 0; i < 6; i++) {
			...
			for (int j = 0; j < pathSizes[i]; j++) {
				...
				if (std::abs(PATH_OP_OP(i, pathSizes[i] - 1, 1))
				    >= 1e-9) { // the final value is not 0 (otherwise always absolute values are considered)
		...
		if (lenPe == 0) {
			...
			for (int i = 0; i < lenPe; i++) {
				stressBasedSimulation = stressBasedSimulation && PATH_OP_OP(pe(i), 1, 1) < 1e9;
			}
		}
	}
	...
	for (int i = 0; i < lenPe; i++) {
		...
		if (pathSizes[j] == 1) { // path has only one part (only final values are prescribed)
			...
			const Real dValue = PATH_OP_OP(j, pathsCounter[j], 1)
			        - PATH_OP_OP(j, pathsCounter[j] - 1, 1); // strain difference at the respective part of the path
			strainRate(j) = dValue / (dProgress * nSteps * dt); // ideal strain rate in respect of dSteps and dValue
		} else {
			strainRate(j) = 0;
		}
	...
	for (int i = 0; i < lenPs; i++) {
		...
		if (pathSizes[j] == 1) { // path has only one part (only final values are prescribed)
			...
			const Real dProgress = PATH_OP_OP(j, pathsCounter[j], 0)
			        - PATH_OP_OP(j, pathsCounter[j] - 1, 0); //  progress difference of respective part of the path
			const Real dValue = PATH_OP_OP(j, pathsCounter[j], 1)
			        - PATH_OP_OP(j, pathsCounter[j] - 1, 1); // stress difference at the respective part of the path
			stressRate(j) = dValue / (dProgress * nSteps * dt); // ideal stress rate in respect of dSteps and dValue
		} else {
			stressRate(j) = 0;
		}
	...
	*/
	if (lenPs > 0) { // if at least 1 stress component is prescribed (otherwise prescribed strain is applied in all 6 directions
		if (progress == 0
		    && stressBasedSimulation) { // the very first step, use compliance estimation (compliance matrix for elastic isotropic material)
			...
			for (int i = 0; i < lenPs; i++) {
				...
				for (int j = 0; j < lenPs; j++) {
					strainRate(ps(i)) += complianceEstimation[ps(i)][ps(j)] * stressRate[ps(j)];
				}
				for (int j = 0; j < lenPe; j++) {
					strainRate(ps(i)) += complianceEstimation[ps(i)][ps(j)] * stressRate[ps(j)];
				}
			}
