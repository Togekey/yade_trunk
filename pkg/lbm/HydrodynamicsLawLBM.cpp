...
inline Vector3i vect3rToVect3i(Vector3r vect)
{
	Vector3i newvect((int)vect[0], (int)vect[1], (int)vect[2]);
	return (newvect);
}
...
bool HydrodynamicsLawLBM::isActivated()
{
	...
	if (EngineIsActivated) {
		if ((DEM_ITER % DemIterLbmIterRatio == 0) && (DEM_ITER != DemIterLbmIterRatio)) {
			...
			if (applyForcesAndTorques)
				CalculateAndApplyForcesAndTorquesOnBodies(false, true);
			return (false);
		}
	} else
		return (false);
	...
	if (firstRun) {
		...
		FOREACH(const shared_ptr<Body>& b, *scene->bodies)
		{
			...
			if (b->shape->getClassName() == "Box") {
				...
				if (ext[0] < halfWallthickness)
					halfWallthickness = ext[0];
				if (ext[1] < halfWallthickness)
					halfWallthickness = ext[1];
				if (ext[2] < halfWallthickness)
					halfWallthickness = ext[2];
				NB_WALLS++;
				tmpbody.setAsBox();
			...
			if (b->shape->getClassName() == "Sphere") {
				...
				if (b->isDynamic()) {
					...
					if (!strcmp(model.c_str(), "d2q9"))
						...
						Vo += 4. / 3. * Mathr::PI * (r * r * r);
					if (initVbCutOff)
						VbCutOff = max(VbCutOff, b->state->vel.norm() + r * b->state->angVel.norm());
		...
		if (!strcmp(model.c_str(), "d2q9")) {
			...
			for (int aa = 1; aa <= 4; aa++)
				w.push_back(1.0 / 9.0);
			for (int aa = 5; aa <= 8; aa++)
				w.push_back(1.0 / 36.0);
			...
			cerr << "This model is not implemented yet: " << model << endl;
			exit(-1);
		}
		...
		if (res >= 0 && res < ll)
			COMPRESS_DATA = true;
		res = LBMSavedData.find("ObservedPtc");
		if (res >= 0 && res < ll)
			SAVE_OBSERVEDPTC = true;
		res = LBMSavedData.find("observedptc");
		if (res >= 0 && res < ll)
			SAVE_OBSERVEDPTC = true;
		res = LBMSavedData.find("observedPtc");
		if (res >= 0 && res < ll)
			SAVE_OBSERVEDPTC = true;
		res = LBMSavedData.find("ObservedNode");
		if (res >= 0 && res < ll)
			SAVE_OBSERVEDNODE = true; // does not activate any recording in a a file currently
		res = LBMSavedData.find("observednode");
		if (res >= 0 && res < ll)
			SAVE_OBSERVEDNODE = true;
		res = LBMSavedData.find("observedNode");
		if (res >= 0 && res < ll)
			SAVE_OBSERVEDNODE = true;
		...
		if (res >= 0 && res < ll) {
			SAVE_SPHERES = true;
			CreateDemDir = true;
		} // To save spheres_* file only if it is required by the operator
		res = LBMSavedData.find("Spheres");
		if (res >= 0 && res < ll) {
			SAVE_SPHERES = true;
			CreateDemDir = true;
		...
		if ((ObservedPtc == -1) && (NB_GRAINS > 0) && (SAVE_OBSERVEDPTC))
			ObservedPtc = IdFirstSphere; //Condition If(SAVE_OBSERVEDPTC) is added to save observedPtc only if it is required by the operator
		if ((SAVE_OBSERVEDPTC) && ((unsigned)ObservedPtc >= LBbodies.size())) {
			cerr << "Error: ObservedPtc>bodies.size()" << endl;
			exit(-1);
		}
		if ((SAVE_SPHERES) && (NB_GRAINS < 1)) {
			cerr << "Error: saving of sphere properties is switched on whereas NB_GRAINS<1" << endl;
			exit(-1);
		}
		...
		cerr << "Ny= " << Ny << " "
		     << " Nx*Ly0/Lx0=" << Nx * Ly0 / Lx0 << endl;
		cerr << "Wallthickness= " << Wallthickness << " "
		     << " dx=" << dx << endl;

		/*-------------------------------------------------------*/
		/* Verification of wall positionning in positive regions */
		/*-------------------------------------------------------*/
		if (sWallYm->se3.position.y() - halfWallthickness < 0.) {
			cerr << "Wall position error: please avoid negatives region (Y)" << endl;
			exit(-1);
		}
		if (sWallXm->se3.position.x() - halfWallthickness < 0.) {
			cerr << "Wall position error: please avoid negatives region (X)" << endl;
			exit(-1);
		}
		if ((dim == 3) && (sWallZm->se3.position.z() - halfWallthickness < 0.)) {
			cerr << "Wall position error: please avoid negatives region (Z)" << endl;
			exit(-1);
		}
		...
		if ((SaveMode != 1) && (SaveMode != 2)) {
			cerr << "Warning unknown SaveMode." << SaveMode << endl;
			exit(-1);
		}
		if ((SaveMode == 1) && (IterSave <= 0)) {
			cerr << "Warning SaveMode==1 and IterSave<=0." << endl;
			exit(-1);
		}
		if ((SaveMode == 2) && (TimeSave <= 0)) {
			cerr << "Warning SaveMode==2 and TimeSave<=0." << endl;
			exit(-1);
		}
		/*---------------------------------------------------------------*/
		...
		for (int nidx = 0; nidx < Nx * Ny; nidx++) {
			...
			for (int dndx = 0; dndx < NbDir; dndx++) {
				...
				if (((I == i) && (J == j)) || (I == -1) || (J == -1) || (I == Nx) || (J == Ny)) {
					...
					nodes[nidx].neighbour_id.push_back(I + J * Nx);
				}
			}
			if (j_update) {
				j++;
		...
		for (int nidx = 0; nidx < Nx * Ny; nidx++) {
			...
			for (int dndx = 0; dndx < NbDir; dndx++) {
				...
				if ((!strcmp(model.c_str(), "d2q9")) && ((dndx == 1) || (dndx == 2) || (dndx == 5) || (dndx == 6))) {
					...
					if (bb.nid2 != -1)
						nodes[bb.nid2].links_id[opp[dndx]] = link_id;
					...
					cerr << "ERROR: Unknow model type: " << model << endl;
					exit(-1);
				}
			}
		}
		...
		if ((ConvergenceThreshold == -1) || (ConvergenceThreshold == 0))
			use_ConvergenceCriterion = false;
		else {
			use_ConvergenceCriterion = true;
			ErrorCriterion           = int(std::round(ConvergenceThreshold));
		}
		/*------------------------------------------------------------------*/
		/*------------------------ LBM PARAMETERS --------------------------*/
		/*------------------------------------------------------------------*/
...
#pragma omp parallel for
	...
	for (int nidx = 0; nidx < Nx * Ny; nidx++) {
		...
		if (firstRun) {
			nodes[nidx].wasObstacle = nodes[nidx].isObstacle;
		}
	}
		...
		if (b->shape->getClassName() == "Sphere") {
			...
			for (int ii = int(std::round(posMin[0])) - 1; ii <= int(std::round(posMax[0])) + 1; ii++)
				for (int jj = int(std::round(posMin[1])) - 1; jj <= int(std::round(posMax[1])) + 1; jj++) {
					...
					if ((ii < -1) || (ii > Nx) || (jj < -1) || (jj > Ny))
						continue;
					if (LBbodies[id].radius < Rmin)
						Rmin = LBbodies[id].radius;
					int nidx = ii + jj * Nx;
			...
			if ((removingCriterion != 0) && (b->isDynamic())) {
				switch (removingCriterion) {
					...
					case 2:
						...
						if ((LBbodies[id].vel.norm() > VelocityThreshold)
						    || (LBbodies[id].pos.x() > (invdx * (sWallXp->se3.position.x()) - 2. * LBbodies[id].radius))) {
							IdOfNextErodedPtc.push_back(id);
						}
						break;
				...
				if (nodes[nidx].neighbour_id[n] != -1) {
					...
					if (nodes[nidx].isObstacle != nodes[nidx2].isObstacle) {
						...
						if (LBbodies[BodyId].isPtc()) {
							...
							if (links[lid].VbMid.norm() < VbCutOff)
...
#pragma omp parallel for
	...
	if (firstRun) {
		...
		/*------------------------------- MODE -----------------------------*/
		/*------------------------------------------------------------------*/
		if ((IterSubCyclingStart <= 0) && (IterMax > 1))
			MODE = 1;
		if ((IterSubCyclingStart <= 0) && (IterMax == 1))
			MODE = 2;
		if (IterSubCyclingStart > 0)
			MODE = 3;

		/*------------------------------------------------------------------*/
		/*---------------------------- SUBCYCLING --------------------------*/
		/*------------------------------------------------------------------*/
		if (MODE == 3 && IterMax < IterSubCyclingStart) {
			cerr << "Exit because Itermax<IterSubCyclingStart" << endl;
			exit(-1);
		}
		if (DemIterLbmIterRatio == -1) {
			if (MODE == 1)
				...
				DemIterLbmIterRatio = int(dt / DEMdt0);
		...
		if (SaveMode == 2) {
			...
			if (TimeSave < dt) {
				cerr << "Warning SaveMode==2 and TimeSave<dt." << endl;
				exit(-1);
			}
		}
		writelogfile();
...
#endif
		for (int nidx = 0; nidx < Nx * Ny; nidx++) {
			if (nodes[nidx].checkIsNewObstacle()) {
				newObstacleCells_couter++;
			} else {
				if (nodes[nidx].checkIsNewFluid()) {
					newFluidCells_couter++;
				}
			}
			...
			for (int dndx = 0; dndx < NbDir; dndx++) {
				...
				if ((nodes[nidx].isFluidBoundary) && (nodes[nidx].neighbour_id[dndx] != -1)) {
					...
					if (!nodes[ns].isObstacle)
						nodes[nidx].IsolNb = nodes[nidx].IsolNb - 1;
					if (nodes[nidx].IsolNb < 0) {
						cerr << "isolNb<0" << endl;
						exit(-1);
					}
				}
			...
			if (nodes[nidx].body_id == -1)
				if (VmaxC < c * nodes[nidx].velb.norm())
					VmaxC = c * nodes[nidx].velb.norm();
			if (VminC > c * nodes[nidx].velb.norm())
				VminC = c * nodes[nidx].velb.norm();
			if (RhomaxC < Rho * nodes[nidx].rhob)
				RhomaxC = Rho * nodes[nidx].rhob;
			if (RhominC > Rho * nodes[nidx].rhob)
				RhominC = Rho * nodes[nidx].rhob;
			if (!nodes[nidx].isObstacle)
				VmeanFluidC += c * nodes[nidx].velb.norm();
		}
...
#pragma omp parallel for
		...
		for (unsigned int lid = 0; lid < links.size(); lid++) {
			...
			if (!links[lid].PointingOutside) {
				...
				nodes[nidx2].fpostcol[dndx2]
				        = nodes[nidx2].fpostcol[dndx2] + (nodes[nidx2].rhob * w[dndx2]) / c2 * eib[dndx2].dot(CstBodyForce);
			}
			...
			if (links[lid].PointingOutside) {
				...
				I = nodes[nidx1].i
				        + int(std::round(
				                  eib[dndx1]
				                          .x())); // XXX: this is suspicious. Anyway, you have to select the int(…) conversion policy: std::floor, std::ceil, std::round
				J = nodes[nidx1].j
				        + int(std::round(
				                  eib[dndx1]
			...
			if ((MODE == 2) || ((MODE == 3) && (IterMax == 1))) {
				links[lid].ReinitDynamicalProperties();
			}
		}
		...
		if (LBM_ITER % IterPrint == 0) {
			...
			if (ObservedPtc != -1) {
				Vector3r tmp = 2. * Rho * c2 * dx * LBbodies[ObservedPtc].force;
				cerr << "| \t\t\t\t\t | F (N) \t: " << tmp << "\t " << endl;
				tmp = LBbodies[ObservedPtc].pos * dx;
				cerr << "| \t\t\t\t\t | pos (m) \t: " << tmp << "\t " << endl;
				tmp = LBbodies[ObservedPtc].vel * c;
				cerr << "| \t\t\t\t\t | VPtc (m/s)\t: " << tmp << "\t \t\t\t\t " << endl;
			}
		...
		if (((iter % (IterSave * DemIterLbmIterRatio) == 0) || (firstRun)) && MODE == 1) {
			if (((iter % (IterSave * SaveGridRatio * DemIterLbmIterRatio) == 0) || (firstRun)) && MODE == 1)
				save(iter, dt);
			saveStats(iter, dt);
			if (SAVE_OBSERVEDPTC) {
			...
			if (SAVE_OBSERVEDNODE)
				saveObservedNode(iter, dt);
		}


		if ((IterSubCyclingStart > 0) && (iter + 1 >= IterSubCyclingStart)) {
			modeTransition();
		}
	...
	/*--------------------------------------------------------------------------------*/
	if (applyForcesAndTorques)
		CalculateAndApplyForcesAndTorquesOnBodies(true, true);
	...
	if (((DEM_ITER % (IterSave * DemIterLbmIterRatio) == 0) || (firstRun)) && IterMax == 1) {
		if (((DEM_ITER % (IterSave * SaveGridRatio * DemIterLbmIterRatio) == 0) || (firstRun)) && IterMax == 1)
			save(DEM_ITER, DEMdt);
		saveStats(DEM_ITER, DEMdt);
		//saveEroded(DEM_ITER,DEMdt);
		if (SAVE_OBSERVEDPTC)
			saveObservedPtc(DEM_ITER, DEMdt);
		if (SAVE_OBSERVEDNODE)
			saveObservedNode(DEM_ITER, DEMdt);
	}
}
...
void HydrodynamicsLawLBM::save(int iter_number, Real /*timestep*/)
{
	...
	/*--- VELOCITIES ---*/
	if (SAVE_VELOCITY) {
		Vfile_name << lbm_dir << "/V"
		           << "_";
		Vfile_name.width(10);
		Vfile_name.fill('0');
		Vfile_name << iter_number;
	...
	if (SAVE_VELOCITYCOMP) {
		Vxfile_name << lbm_dir << "/Vx"
		            << "_";
		Vxfile_name.width(10);
		Vyfile_name << lbm_dir << "/Vy"
		            << "_";
		Vyfile_name.width(10);

		Vxfile_name.fill('0');
		Vxfile_name << iter_number;
		Vyfile_name.fill('0');
		Vyfile_name << iter_number;

		Vxfile.open(Vxfile_name.str().c_str());
		Vyfile.open(Vyfile_name.str().c_str());
	...
	/*--- FORCES ---*/
	if (SAVE_FORCES) {
		FxLBMfile_name << lbm_dir << "/Fx"
		               << "_";
		FxLBMfile_name.width(10);
		FyLBMfile_name << lbm_dir << "/Fy"
		               << "_";
		FyLBMfile_name.width(10);
		MzLBMfile_name << lbm_dir << "/Mz"
		               << "_";
		MzLBMfile_name.width(10);

		FxLBMfile_name.fill('0');
		FxLBMfile_name << iter_number;
		FyLBMfile_name.fill('0');
		FyLBMfile_name << iter_number;
		MzLBMfile_name.fill('0');
		MzLBMfile_name << iter_number;
	...
	/*--- DENSITY ---*/
	if (SAVE_RHO) {
		Rhofile_name << lbm_dir << "/Rho"
		             << "_";
		Rhofile_name.width(10);
		Rhofile_name.fill('0');
		Rhofile_name << iter_number;
	...
	/*--- BODIES ---*/
	if (SAVE_BODIES) {
		Bodiesfile_name << lbm_dir << "/Bodies"
		                << "_";
		Bodiesfile_name.width(10);
		Bodiesfile_name.fill('0');
		Bodiesfile_name << iter_number;
	...
	/*--- BOUNDARY NODES ---*/
	if (SAVE_NODEBD) {
		NodeBoundaryfile_name << lbm_dir << "/NodeBoundary"
		                      << "_";
		NodeBoundaryfile_name.width(10);
		NodeBoundaryfile_name.fill('0');
		NodeBoundaryfile_name << iter_number;
	...
	/*--- NEW NODES ---*/
	if (SAVE_NODEISNEW) {
		NewNodefile_name << lbm_dir << "/NewNode"
		                 << "_";
		NewNodefile_name.width(10);
		NewNodefile_name.fill('0');
		NewNodefile_name << iter_number;
	...
	if (saveLBMdata) { //Condition to avoid to loop over all the lbm node if no lbm data are saved but only spheres data
		...
		for (int nidx = 0; nidx < Nx * Ny; nidx++) {
			if (SAVE_BODIES)
				Bodiesfile << nodes[nidx].body_id << " ";
			if (SAVE_VELOCITY)
				Vfile << c * nodes[nidx].velb.norm() << " ";
			if (SAVE_VELOCITYCOMP) {
				Vxfile << c * nodes[nidx].velb.x() << " ";
				Vyfile << c * nodes[nidx].velb.y() << " ";
			}
			if (SAVE_FORCES) {
			...
			if (SAVE_RHO)
				Rhofile << Rho * nodes[nidx].rhob << " ";
			if (SAVE_NODEBD) {
				int tmp = 0;
				if (nodes[nidx].isObstacleBoundary)
					tmp = nodes[nidx].isObstacleBoundary;
				if (nodes[nidx].isFluidBoundary)
					tmp = -1;
			...
			if (SAVE_NODEISNEW) {
				int tmp = 0;
				if (nodes[nidx].isNewObstacle)
					tmp = nodes[nidx].isNewObstacle;
				if (nodes[nidx].isNewFluid)
					tmp = -1;
				NewNodefile << tmp << " ";
			}
	...
	if (SAVE_BODIES) {
		Bodiesfile.close();
		cmd << " " << Bodiesfile_name.str().c_str();
	}
	if (SAVE_VELOCITY) {
		Vfile.close();
		cmd << " " << Vfile_name.str().c_str();
	}
	if (SAVE_VELOCITYCOMP) {
		Vxfile.close();
		Vyfile.close();
		cmd << " " << Vxfile_name.str().c_str() << " " << Vyfile_name.str().c_str();
	}
	if (SAVE_FORCES) {
		FxLBMfile.close();
		FyLBMfile.close();
	...
	if (SAVE_RHO) {
		Rhofile.close();
		cmd << " " << Rhofile_name.str().c_str();
	}
	if (SAVE_NODEBD) {
		NodeBoundaryfile.close();
		cmd << " " << NodeBoundaryfile_name.str().c_str();
	}
	if (SAVE_NODEISNEW) {
		NewNodefile.close();
		cmd << " " << NewNodefile_name.str().c_str();
	}
	//cmd<<"&";
	if (COMPRESS_DATA) {
		if (std::system(cmd.str().c_str()))
			cerr << "bzip error" << endl;
	}
		...
		for (unsigned int l = 0; l < LBbodies.size(); l++) {
			if (LBbodies[l].isPtc()) {
				if ((!LBbodies[l].isEroded) && (LBbodies[l].saveProperties)) {
					spherefile << l << " "; /* Id*/
					spherefile << dx * LBbodies[l].pos.x() << " " << dx * LBbodies[l].pos.y() << " " << dx * LBbodies[l].pos.z()
					           << " "; /* x, y, z*/
					spherefile << dx * LBbodies[l].radius << " "; /* r*/
					spherefile << c * LBbodies[l].vel.x() << " " << c * LBbodies[l].vel.y() << " " << c * LBbodies[l].vel.z()
					           << " "; /* Vx, Vy, Vz*/
					spherefile << invdt * LBbodies[l].AVel.x() << " " << invdt * LBbodies[l].AVel.y() << " " << invdt * LBbodies[l].AVel.z()
					           << " "; /* Wx, Wy, Wz*/
...
void HydrodynamicsLawLBM::saveObservedNode(int /*iter_number*/, Real /*timestep*/) { return; }
/*
...
void HydrodynamicsLawLBM::createNewFiles()
{
	...
	if (SAVE_OBSERVEDNODE) { //Condition to create observedNode file only if the recording is required by the operator
		ofstream file5(ObservedNodeFile.c_str());
		file5.close();
	}
...
void HydrodynamicsLawLBM::writelogfile()
{
	...
	if (NB_GRAINS > 0)
		file << "\t NbNodePerPtc= " << NbParticleNodes / NB_GRAINS << endl;
	else
		file << "\t NbNodePerPtc= " << -1 << endl;
	file << "\t Vo= " << Vo << endl;
	file << "Misc :" << endl;
