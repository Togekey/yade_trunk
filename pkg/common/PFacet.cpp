        ...
        const shared_ptr<Interaction>& c)
{
	...
	if (std::abs(dist) > (PFacetradius + sphereRadius) && !c->isReal()
	...
	if (isintriangle) {
		...
		if (connnum != -1) {
			...
			for (int i = 0; i < 3; i++) {
				if (std::abs(branch[i]) < 1e-14)
					branch[i] = 0.0;
				if (std::abs(branchN[i]) < 1e-14)
					branchN[i] = 0.0;
			}
        ...
        const shared_ptr<Interaction>& c)
{
	...
	if (istriangleNodes1P2) {
		...
		if (std::abs(normal1seg21seg11) == 1) {
			c1 = ids2[0];
			c2 = ids1[0];
		}
		if (std::abs(normal1seg21seg12) == 1) {
			c1 = ids2[0];
			c2 = ids1[1];
		}
		if (std::abs(normal1seg21seg13) == 1) {
			c1 = ids2[0];
			c2 = ids1[2];
		}

		if (std::abs(normal1seg22seg11) == 1) {
			c1 = ids2[1];
			c2 = ids1[0];
		}
		if (std::abs(normal1seg22seg12) == 1) {
			c1 = ids2[1];
			c2 = ids1[1];
		}
		if (std::abs(normal1seg22seg13) == 1) {
			c1 = ids2[1];
			c2 = ids1[2];
		}

		if (std::abs(normal1seg23seg11) == 1) {
			c1 = ids2[2];
			c2 = ids1[0];
		}
		if (std::abs(normal1seg23seg12) == 1) {
			c1 = ids2[2];
			c2 = ids1[1];
		}
		if (std::abs(normal1seg23seg13) == 1) {
			c1 = ids2[2];
			c2 = ids1[2];
		}
