...
//!			O-O
bool Ig2_GridNode_GridNode_GridNodeGeom6D::go(
        ...
        const shared_ptr<Interaction>& c)
{
	...
	if (Ig2_Sphere_Sphere_ScGeom::go(cm1, cm2, state1, state2, shift2, force, c)) { //the 3 DOFS from ScGeom are updated here
		...
		if (updateRotations)
			YADE_PTR_CAST<GridNodeGeom6D>(c->geom)->precomputeRotations(state1, state2, isNew, creep);
...
bool Ig2_GridNode_GridNode_GridNodeGeom6D::goReverse(
        const shared_ptr<Shape>&       cm1,
        const shared_ptr<Shape>&       cm2,
        const State&                   state1,
        const State&                   state2,
...
//!			\\//
bool Ig2_GridConnection_GridConnection_GridCoGridCoGeom::go(
        const shared_ptr<Shape>& cm1,
        const shared_ptr<Shape>& cm2,
        const State& /*state1*/,
        const State& /*state2*/,
        ...
        const shared_ptr<Interaction>& c)
{
	...
	if (isNew) {
		...
		scm = YADE_PTR_CAST<GridCoGridCoGeom>(c->geom);
	...
	scm->radius1               = conn1->radius;
	scm->radius2               = conn2->radius;
...
bool Ig2_GridConnection_GridConnection_GridCoGridCoGeom::goReverse(
        const shared_ptr<Shape>&       cm1,
        const shared_ptr<Shape>&       cm2,
        const State&                   state1,
        const State&                   state2,
...
//!			O/
bool Ig2_Sphere_GridConnection_ScGridCoGeom::go(
        const shared_ptr<Shape>& cm1,
        const shared_ptr<Shape>& cm2,
        const State&             state1,
        const State& /*state2*/,
...
{ // Useful variables :
	...
	if (relPos <= 0) { // if the sphere projection is BEFORE the segment ...
		if (gridNo1->ConnList.size() > 1) { //	if the node is not an extremity of the Grid (only one connection)
			for (int unsigned i = 0; i < gridNo1->ConnList.size(); i++) { // ... loop on all the Connections of the same Node ...
				...
				Vector3r segtCandidate1
				        = GC->node1->state->pos - gridNo1St->pos; // (be sure of the direction of segtPrev to compare relPosPrev.)
					...
					if (intr && intr->isReal()) {
						...
						if (!(intrGeom->isDuplicate == 1)) { //skip contact.
							if (isNew)
								...
								scm->isDuplicate = 1; /*cout<<"Declare "<<c->id1<<"-"<<c->id2<<" as duplicated."<<endl;*/
							}
						}
					}
	...
	if (isNew && (dist > (sphere->radius + gridCo->radius)))
		return false;
	...
	scm->fictiousState.angVel = ((1 - relPos) * gridNo1St->angVel + relPos * gridNo2St->angVel).dot(segt / len) * segt / len //twist part : interpolated
...
bool Ig2_Sphere_GridConnection_ScGridCoGeom::goReverse(
        const shared_ptr<Shape>&       cm1,
        const shared_ptr<Shape>&       cm2,
        const State&                   state1,
        const State&                   state2,
...
bool Law2_ScGridCoGeom_CohFrictPhys_CundallStrack::go(shared_ptr<IGeom>& ig, shared_ptr<IPhys>& ip, Interaction* contact)
{
	...
	if (contact->isFresh(scene) && geom->isDuplicate != 2)
