...
void Gl1_Sphere::go(const shared_ptr<Shape>& cm, const shared_ptr<State>&, bool wire2, const GLViewInfo&)
{
	...
	if (circleView) {
		bool somethingChanged
		        = (std::abs(quality - prevQuality) > 0.001 || prevDisplayMode != "torus" || prevCircleAllowedRotationAxis != circleAllowedRotationAxis);
		if (somethingChanged) {
			...
			glNewList(glGlutSphereList, GL_COMPILE)
				...
				glutSolidTorus(
				        ...
				        int(std::round(quality * glutStacks)),
				        int(std::round(quality * glutSlices))); //generate torus
			glEndList();
		}
		...
		if (wire || wire2) {
			...
			bool somethingChanged
			        = (std::abs(quality - prevQuality) > 0.001 || glIsList(glStripedSphereList) != GL_TRUE || prevDisplayMode != "sphere");
			if (somethingChanged) {
				initStripedGlList();
				initGlutGlList();
				prevQuality     = quality;
