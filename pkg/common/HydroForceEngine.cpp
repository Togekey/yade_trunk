		...
		if (sphere) {
			...
			if ((p < nCell) && (p >= 0)) {
				...
				if ((p + intRadius < nCell) && (p - intRadius > 0) && (lift == true)) {
					double vRelTop = vxFluid[p + 1 + intRadius]
...
void HydroForceEngine::fluidResolution(double tfin, double dt)
{
	...
	double phi_nodej, termeVisco_j, termeVisco_jp1, termeTurb_j, termeTurb_jp1, viscof, dz, sum, phi_lim, dudz, ustar, fluidHeight, urel, urel_bound, Re,
	        eps, ff, ffold, delta, dddiv, ejm1, phijm1, upjm1, ejp1, phijp1, upjp1, secondMemberPart;
	vector<double> sig(nCell, 0.0), dsig(nCell - 1, 0.), viscoeff(nCell, 0.), ufn(nCell + 1, 0.), wallFriction(nCell - 1, 0.), viscoft(nCell, 0.),
	...
	while (time <= tfin) {
		...
		for (j = 0; j < nCell - 1; j++) {
			...
			s[j + 1] = ufn[j + 1] * epsilon[j] + epsilon[j] * dt * std::abs(gravity[0]) + dt * taufsi[j] * vxPart[j] + secondMemberPart
