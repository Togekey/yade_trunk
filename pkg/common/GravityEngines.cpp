...
void GravityEngine::action()
{
	...
	YADE_PARALLEL_FOREACH_BODY_BEGIN(const shared_ptr<Body>& b, scene->bodies)
	{
		...
		if (trackEnergy)
			scene->energy->add(-gravity.dot(b->state->vel) * b->state->mass * dt, "gravWork", fieldWorkIx, /*non-incremental*/ false);
	}
...
void CentralGravityEngine::action()
{
	...
	{
		if (b->isClump() || b->getId() == centralBody)
			continue; // skip clumps and central body
		if (mask != 0 && !b->maskCompatible(mask))
			continue;
		...
		if (reciprocal)
			scene->forces.addForce(centralBody, -F * toCenter);
	}
}
...
Vector2i HdapsGravityEngine::readSysfsFile(const string& name)
{
	...
	if (!f.is_open())
		throw std::runtime_error(("HdapsGravityEngine: unable to open file " + name).c_str());
	f.read(buf, 256);
	f.close();
	const boost::regex re("\\(([0-9+-]+),([0-9+-]+)\\).*");
	boost::cmatch      matches;
	if (!boost::regex_match(buf, matches, re))
		throw std::runtime_error(("HdapsGravityEngine: error parsing data from " + name).c_str());
...
void HdapsGravityEngine::action()
{
	...
	if (now - lastReading > 1e-3 * msecUpdate) {
		...
		lastReading = now;
		a -= calibrate;
		if (std::abs(a[0] - accel[0]) > updateThreshold)
			accel[0] = a[0];
		if (std::abs(a[1] - accel[1]) > updateThreshold)
			accel[1] = a[1];
