...
public:
	PeriodicVertexInfo& operator=(const CVector& u)
	{
		CVector::operator=(u);
		return *this;
	}
	PeriodicVertexInfo& operator=(const float& scalar)
	{
		s = scalar;
		return *this;
	}
	PeriodicVertexInfo& operator=(const unsigned int& id)
	{
		i = id;
		return *this;
	}
...
typedef TemplateFlowEngine_FlowEngine_PeriodicInfo<
        PeriodicCellInfo,
        PeriodicVertexInfo,
...
public:
	...
	YADE_CLASS_BASE_DOC_ATTRS_INIT_CTOR_PY(
	        PeriodicFlowEngine,
	        FlowEngine_PeriodicInfo,
	        "A variant of :yref:`FlowEngine` implementing periodic boundary conditions. The API is very similar.",
...
void PeriodicFlowEngine::action()
{
	...
	if (first) {
		...
		if (solver->errorCode > 0) {
			LOG_INFO("triangulation error, pausing");
...
void PeriodicFlowEngine::triangulate(FlowSolver& flow)
{
	...
	{
		if (!b.exists || !b.isSphere || b.id == ignoredBody)
			continue;
		Vector3i period;
		Vector3r wpos;
		...
		if (vh0 == NULL) {
			flow.errorCode = 2;
			LOG_ERROR("Vh NULL in PeriodicFlowEngine::triangulate(), check input data");
			continue;
		}
		for (int k = 0; k < 3; k++)
			vh0->info().period[k] = -period[k];
			...
			for (i[1] = -1; i[1] <= 1; i[1]++)
				for (i[2] = -1; i[2] <= 1; i[2]++) {
					if (i[0] != 0 || i[1] != 0 || i[2] != 0) { // middle; already rendered above
						...
						pmin = pos2 - halfSize;
						pmax = pos2 + halfSize;
						if ((pmin[0] <= cellSize[0]) && (pmax[0] >= 0) && (pmin[1] <= cellSize[1]) && (pmax[1] >= 0)
...
Real PeriodicFlowEngine::volumeCell(CellHandle cell)
{
	...
	if (!(cell->info().volumeSign))
		cell->info().volumeSign = (volume > 0) ? 1 : -1;
	return volume;
}
		...
		if (!(cell->vertex(y)->info().isFictious)) {
			...
			if (!solver->boundary(b).useMaxMin)
				Wall_coordinate = wll->state->pos[solver->boundary(b).coordinate]
				        + (solver->boundary(b).normal[solver->boundary(b).coordinate]) * wallThickness / 2.;
			else
				Wall_coordinate = solver->boundary(b).p[solver->boundary(b).coordinate];
		}
	}
	Real Volume = 0.5 * ((V[0] - V[1]).cross(V[0] - V[2]))[solver->boundary(b).coordinate]
	        * (0.33333333333 * (V[0][solver->boundary(b).coordinate] + V[1][solver->boundary(b).coordinate] + V[2][solver->boundary(b).coordinate])
	           - Wall_coordinate);
...
void PeriodicFlowEngine::locateCell(CellHandle baseCell, unsigned int& index, int& baseIndex, FlowSolver& flow, unsigned int count)
{
	...
	if (baseInfo.index > 0 || baseInfo.isGhost)
		return;
	...
	if (baseCell->info().fictious() == 0)
		...
		int  coord    = 0;
		for (int k = 0; k < 4; k++) {
			if (!baseCell->vertex(k)->info().isFictious)
				center += 0.3333333333 * makeVector3r(baseCell->vertex(k)->point().point());
			else {
	...
	if (period[0] != 0 || period[1] != 0 || period[2] != 0) {
		...
		for (int kk = 0; kk < 4; kk++)
			if ((!baseCell->vertex(kk)->info().isGhost) && ((!baseCell->vertex(kk)->info().isFictious)))
				checkC = true;
		if (checkC) {
			bool checkV = true;
			for (int kk = 0; kk < 4; kk++) {
				...
				for (int jj = 0; jj < 4; jj++)
					if (baseCell->vertex(kk)->info().id() == ch->vertex(jj)->info().id())
						checkV = true;
				if (!checkV) {
					cerr << "periodicity is broken" << endl;
					for (int jj = 0; jj < 4; jj++)
						cerr << baseCell->vertex(jj)->info().id() << " ";
					cerr << " vs. ";
					for (int jj = 0; jj < 4; jj++)
						cerr << ch->vertex(jj)->info().id() << " ";
					cerr << endl;
				}
			}
		} else {
		...
		if (baseInfo.baseIndex < 0) {
			...
			if (!baseInfo.Pcondition)
				baseInfo.index = ++index;
		}
	}
}
...
{ //FIXME: replace by the non-periodic version
	...
	if (debug)
		cout << "Updated volumes, total =" << totVol << ", dVol=" << totDVol << " " << totVol0 << " " << totVol1 << endl;
}
...
void PeriodicFlowEngine::initializeVolumes(FlowSolver& flow)
{
	...
	FOREACH(CellHandle & cell, flow.tesselation().cellHandles)
	{
		...
		if (flow.fluidBulkModulus > 0 || iniVoidVolumes) {
			cell->info().invVoidVolume()
			        = 1. / (max(minimumPorosity * cell->info().volume(), std::abs(cell->info().volume()) - flow.volumeSolidPore(cell)));
		}
	}
	if (debug)
...
void PeriodicFlowEngine::buildTriangulation(double pZero, FlowSolver& flow)
{
	...
	addBoundary(flow);
	if (debug)
		cout << endl << "Added boundaries------" << endl << endl;
	triangulate(flow);
	if (debug)
	...
	if (debug)
		cout << endl << "boundaryConditions------" << endl << endl;
	flow.initializePressure(pZero);
	if (debug)
		cout << endl << "initializePressure------" << endl << endl;
	...
	for (FiniteCellsIterator cell = Tes.Triangulation().finite_cells_begin(); cell != cellend; cell++) {
		...
		if (flow.errorCode > 0) {
			LOG_ERROR("problem here, flow.errorCode>0");
			return;
		}
		//Fill this vector than can be later used to speedup loops
		if (!cell->info().isGhost)
			Tes.cellHandles[cell->info().baseIndex] = cell;
	...
	if (debug)
		cout << endl << "locateCell------" << endl << endl;
	...
	if (!first && !multithread && (useSolver == 0 || fluidBulkModulus > 0 || doInterpolate))
		flow.interpolate(flow.T[!flow.currentTes], Tes);
	// 	if ( !first && (useSolver==0 || fluidBulkModulus>0)) flow.interpolate ( flow.T[!flow.currentTes], flow.tesselation() );

	if (waveAction)
		flow.applySinusoidalPressure(Tes.Triangulation(), sineMagnitude, sineAverage, 30);

	if (normalLubrication || shearLubrication || viscousShear)
		flow.computeEdgesSurfaces();
	if (debug)
		cout << endl << "end buildTri------" << endl << endl;
}
