...
public:
	...
	DFNCellInfo()
	        : crack(false)
	{
	} /// enable to visualize cracked cells in Paraview
...
public:
	void saveVtk(const char* folder, bool withBoundaries) //FIXME: withBoundaries does nothing in DFNFlow right now. Needed for consistent templating.
	{
		...
		basicVTKwritter vtkfile(
		        (unsigned int)Tri.number_of_vertices() - vtkInfiniteVertices, (unsigned int)Tri.number_of_finite_cells() - vtkInfiniteCells);
		...
		for (FiniteCellsIterator cell = Tri.finite_cells_begin(); cell != Tri.finite_cells_end(); ++cell) {
			...
			if (isDrawable) {
				vtkfile.write_cell(
				        cell->vertex(0)->info().id() - firstReal,
				        cell->vertex(1)->info().id() - firstReal,
				        cell->vertex(2)->info().id() - firstReal,
				        cell->vertex(3)->info().id() - firstReal);
			}
		}
		vtkfile.end_cells();
		...
		if (1) {
			...
			for (FiniteCellsIterator cell = Tri.finite_cells_begin(); cell != Tri.finite_cells_end(); ++cell) {
				...
				if (isDrawable) {
					vtkfile.write_data(
					        cell->info().averageVelocity()[0], cell->info().averageVelocity()[1], cell->info().averageVelocity()[2]);
				}
			}
			vtkfile.end_data();
		}
...
public:
	...
	YADE_CLASS_BASE_DOC_ATTRS_INIT_CTOR_PY(
	        DFNFlowEngine,
	        DFNFlowEngineT,
	        "This is an enhancement of the FlowEngine for intact and fractured rocks that takes into acount pre-existing discontinuities and bond breakage "
	        "between particles. The local conductivity around the broken link is calculated according to parallel plates model",
...
void DFNFlowEngine::trickPermeability(RTriangulation::Finite_edges_iterator& edge, Real aperture)
{
	...
	while (facet1 != facet0) {
		trickPermeability(facet1, aperture, edge);
		facet1++;
	}
	...
	for (; edge != Tri.finite_edges_end(); ++edge) {
		...
		if (interaction && interaction->isReal()) {
			...
			if (jcfpmphys->isOnJoint || jcfpmphys->isBroken) {
				...
				Real residualAperture = jcfpmphys->isOnJoint ? jointsResidualAperture
				                                             : inducedCracksResidualAperture; /// inducedCracksResidualAperture=0. by default
				// 				/// ROBERT
