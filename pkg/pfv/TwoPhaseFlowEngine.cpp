	...
	for (auto cellIt = pCells.begin(); cellIt != pCells.end(); cellIt++) {
		...
		for (int j = 0; j < 4; j++)
			if ((not tes->Triangulation().is_infinite(cell->neighbor(j))) and !cell->neighbor(j)->info().blocked) {
				...
				if (tes->Triangulation().is_infinite(cell->neighbor(j)))
					LOG_WARN("infinite neighbour");
			}
		...
		while (check == false) {
			...
			if (first) {
				...
				if (M.determinant() < 0.0) {
					initialSign = false;
				} //Initial D is negative
				if (M.determinant() > 0.0) {
					initialSign = true;
				} // Initial D is positive
			}

			if (std::abs(M.determinant()) < 1E-100) {
				check = true;
			} //TODO:M.determinant should be converted to dimensionless.
			...
			if (std::abs(tempR - Rin) / Rin < 0.001) {
				check = true;
			}
		}
...
double TwoPhaseFlowEngine::computeMSPRcByPosRadius(
        const Vector3r& posA, const double& rA, const Vector3r& posB, const double& rB, const Vector3r& posC, const double& rC)
{
	...
	if (rmin > rmax) {
		cerr << "WARNING! rmin>rmax. rmin=" << rmin << " ,rmax=" << rmax << endl;
	}
...
double TwoPhaseFlowEngine::bisection(
        const Vector3r& posA, const double& rA, const Vector3r& posB, const double& rB, const Vector3r& posC, const double& rC, double a, double b)
{
	double m = 0.5 * (a + b);
...
double TwoPhaseFlowEngine::computeDeltaForce(
        const Vector3r& posA, const double& rA, const Vector3r& posB, const double& rB, const Vector3r& posC, const double& rC, double r)
{
...
void TwoPhaseFlowEngine::initializeReservoirs()
{
	...
	if (solver->debugOut) {
		cout << "----initializeReservoirs----" << endl;
	}
}
...
double TwoPhaseFlowEngine::getChi(int numberFacets)
{
	if (numberFacets == 0) {
		...
		if (numberFacets == 4) {
			...
			chi = 0.0893 * std::log(float(numberFacets)) + 0.326;
		} //Other pore shapes
		return chi;
	}
}
...
double TwoPhaseFlowEngine::getLambda(int numberFacets)
{
	if (numberFacets == 0) {
		...
		if (numberFacets == 4) {
			...
			lambda = 7.12 * std::pow(numberFacets, -0.89);
		} //Other pore shapes
		return lambda;
	}
}
...
{ //given in radians which is reported as tetha in manuscript Sweijen et al.,
	if (numberFacets == 0) {
		...
		if (numberFacets == 4) {
			DihedralAngle = 1.0 * std::atan(2.0 * std::sqrt(2.0));
		} //Tetrahedra
		else if (numberFacets == 6) {
			DihedralAngle = 1.0 * std::acos(-1.0 / 3.0);
		} //Octahedra
		else if (numberFacets == 8) {
			...
			DihedralAngle = std::acos((-1.0 / 3.0) * std::sqrt(5.0));
		} //Icosahedra
		else if (numberFacets == 20) {
			DihedralAngle = std::acos((-1.0 / 5.0) * std::sqrt(5.0));
		} // Dodecahedron
		else {
...
double TwoPhaseFlowEngine::getConstantC3(CellHandle cell)
{
	...
	if (cell->info().numberFacets == 4) {
		c1 = 8.291;
	}
	if (cell->info().numberFacets == 6) {
		c1 = 2.524;
	}
	if (cell->info().numberFacets == 8) {
		c1 = 2.524;
	}
	if (cell->info().numberFacets == 10) {
		c1 = 6.532;
	}
	if (cell->info().numberFacets == 12) {
		c1 = 6.087;
	}
	if (cell->info().numberFacets == 20) {
		c1 = 0.394;
	}
...
{
	if (pw == 0) {
		std::cout << endl << "Error! water pressure is zero, while computing capillary pressure ... cellId= " << cell->info().id;
	}
...
double TwoPhaseFlowEngine::poreSaturationFromPcS(CellHandle cell, double pw)
{
	...
	if (s > 1.0 || s < 0.0) {
		std::cout << "Error, saturation from Pc(S) curve is not correct: " << s << " " << cell->info().poreId
		          << " log:" << std::log(1.0 + cell->info().thresholdPressure / pw) << " " << (-1.0 * getKappa(cell->info().numberFacets))
...
double TwoPhaseFlowEngine::porePressureFromPcS(CellHandle cell, double /*saturation*/)
{
	...
	if (std::exp(-1 * getKappa(cell->info().numberFacets) * cell->info().saturation) == 1.0) {
	...
	if (pw > 0) {
		std::cout << "Pw is above 0! - error: " << pw << " id=" << cell->info().poreId << " pthr=" << cell->info().thresholdPressure
		          ...
		          << (1.0 - std::exp(-1 * getKappa(cell->info().numberFacets) * cell->info().saturation));
		pw = -1 * cell->info().thresholdPressure;
	}
	if (pw != pw) {
		std::cout << "Non existing capillary pressure!";
	}
...
void TwoPhaseFlowEngine::mergeCells()
{
	...
	for (unsigned int i = 0; i < 110; i++) {
		...
		if (i >= 10) {
			dC = (50.0 - maximumRatioPoreThroatoverPoreBody) / 100.0;
		}
		if (i == 0) {
			dC = 0.0;
		}
						...
						if (cell->neighbor(ngb)->info().mergednr < 20) {
							if (cell->neighbor(ngb)->info().isGhost == false
								...
								if ((cell->info().poreThroatRadius[ngb]
								     / (getChi(cell->info().numberFacets) * std::pow(cell->info().mergedVolume, (1. / 3.))))
								    > criterion) {
	...
	if (debugTPF) {
		cout << endl << "EFFICIENT - RENUMBER MERGEDCELLS -- FROM: " << number << " TO: " << ID + 1;
	}
	for (FiniteCellsIterator cell = tri.finite_cells_begin(); cell != cellEnd; cell++) {
		cell->info().mergedVolume = cell->info().poreBodyVolume;
	}
	...
	for (unsigned int k = 1; k < maxIDMergedCells; k++) {
		...
		for (FiniteCellsIterator cell = tri.finite_cells_begin(); cell != cellEnd; cell++) {
			if (cell->info().mergedID == k) {
				if (summngb < 4) {
					summngb = 4;
				} //Less than 4 throats is not supported -> boundary problems.
...
void TwoPhaseFlowEngine::getMergedCellStats()
{
	...
	for (unsigned int i = 0; i < countFacets.size(); i++) {
		if (debugTPF) {
			cout << endl << "nrFacets: " << (i + 4) << "-count:" << countFacets[i];
		}
		distributionInFacets << (i + 4) << " " << countFacets[i] << '\n';
	}
	for (unsigned int i = 0; i < countMergedNR.size(); i++) {
		if (debugTPF) {
			cout << endl << "nrMergedUnits: " << i + 1 << "-count:" << countMergedNR[i];
		}
				...
				for (unsigned int i = 0; i < 4; i++) {
					if ((cell->info().mergedID != cell->neighbor(i)->info().mergedID
						...
						if (cell->info().poreThroatRadius[i] >= maximumRatioPoreThroatoverPoreBody
							...
							count                            = count + 1;
							cell->info().poreThroatRadius[i] = std::min(
							        (maximumRatioPoreThroatoverPoreBody * getChi(cell->info().numberFacets)
							         * std::pow(cell->info().mergedVolume, (1. / 3.))),
	...
	for (FiniteCellsIterator cell = tri.finite_cells_begin(); cell != cellEnd; cell++) {
		if (cell->info().isFictious == 0) {
			...
			if (cell->info().mergedID == 0) {
				volumeSingleCells = volumeSingleCells + cell->info().poreBodyVolume;
			}
		}
	}
...
void TwoPhaseFlowEngine::calculateResidualSaturation()
{
	...
	for (FiniteCellsIterator cell = tri.finite_cells_begin(); cell != cellEnd; cell++) {
		...
		if (cell->info().poreBodyRadius != 0) {
			cell->info().thresholdPressure = 2.0 * surfaceTension / cell->info().poreBodyRadius;
		}
...
void TwoPhaseFlowEngine::reTriangulate()
{
	...
	if (debugTPF) {
		std::cerr << endl << "Apply retriangulation";
	}
...
void TwoPhaseFlowEngine::readTriangulation()
{
	...
	for (FiniteCellsIterator cell = tri.finite_cells_begin(); cell != cellEnd; cell++) {
		...
		if (cell->info().isFictious) {
			finishedUpdating[cell->info().id] = -1;
		}
		if (!cell->info().isFictious) {
...
void TwoPhaseFlowEngine::assignWaterVolumesTriangulation()
{
	...
	for (unsigned int i = 0; i < index; i++) {
		if (finishedUpdating[i] == 0) {
			double totalArea
			        = solidFractionSpPerTet[i][0] + solidFractionSpPerTet[i][1] + solidFractionSpPerTet[i][2] + solidFractionSpPerTet[i][3];
			for (unsigned int j = 0; j < 4; j++) {
		...
		if (cell->info().saturation == -1) {
			...
			for (unsigned int j = 0; j < 4; j++) {
				vol += leftOverVolumePerSphere[cell->vertex(j)->info().id()]
				        * (std::abs(solver->fractionalSolidArea(cell, j)) / untreatedAreaPerSphere[cell->vertex(j)->info().id()]);
...
void TwoPhaseFlowEngine::equalizeSaturationOverMergedCells()
{
	...
	for (FiniteCellsIterator cell = tri.finite_cells_begin(); cell != cellEnd; cell++) {
		if (cell->info().saturation < 0.0) {
			std::cout << endl << "Error! Before Starting! Negative sat! ... " << cell->info().saturation;
		}
	}
	...
	for (unsigned int k = 1; k < maxIDMergedCells; k++) {
		...
		if (leftOverVolume > 0.0) {
			...
			if (leftOverVolume > 0.0) {
				std::cout << endl << "Error! Left over water volume: " << leftOverVolume;
			}
		}
	...
	for (FiniteCellsIterator cell = tri.finite_cells_begin(); cell != cellEnd; cell++) {
		if (!cell->info().isFictious && cell->info().saturation > 1.0 && cell->info().mergedID == 0) {
			...
			if (leftOverVolume > 0.0) {
				std::cout << "Mass left during remeshing" << leftOverVolume;
			}
		}
	}
...
{
	if (debugTPF) {
		std::cerr << endl << "Set initial condition";
	}
...
void TwoPhaseFlowEngine::setBoundaryConditions()
{
	...
	if (waterBoundaryPressure == 0.0) {
		waterBoundaryPressure = -1 * initialPC;
	}
}
			...
			for (unsigned int j = 0; j < listOfPores[i]->info().poreNeighbors.size(); j++) {
				if (listOfPores[listOfPores[i]->info().poreNeighbors[j]]->info().saturation < 1.0) {
					if (-1.0 * listOfPores[i]->info().p() > listOfPores[i]->info().listOfEntryPressure[j]) {
						...
						double areaWater
						        = listOfPores[i]->info().listOfThroatArea[j] - 3.1415926535 * radiusCurvature * radiusCurvature;
						if (areaWater < 0.0) {
							areaWater = listOfPores[i]->info().listOfThroatArea[j];
						}

						double hydraulicRad = 4.0 * surfaceTension
						        / (-1.0
...
void TwoPhaseFlowEngine::solvePressure()
{
	...
	for (unsigned int i = 0; i < numberOfPores; i++) {
		if (!deformation && hasInterfaceList[i] && listOfFlux[i] != 0.0 && !listOfPores[i]->info().isWResInternal) {
			double ds = -1.0 * scene->dt * (listOfFlux[i])
	...
	if (!deformation && std::abs(boundaryFlux * scene->dt + (waterBefore - waterAfter)) / std::abs(boundaryFlux * scene->dt) > 1e-3
	    && std::abs(boundaryFlux) > 1e-18) { //FIXME test has to optimized for deforming pore units
		std::cerr << endl
		          << "No volume balance! Flux balance: WBFlux:"
		          << std::abs(boundaryFlux * scene->dt + (waterBefore - waterAfter)) / std::abs(boundaryFlux * scene->dt) << " "
	...
	for (unsigned int i = 0; i < numberOfPores; i++) {
		if (deformation) {
			...
			listOfPores[i]->info().poreBodyRadius
			        = getChi(listOfPores[i]->info().numberFacets) * std::pow(listOfPores[i]->info().mergedVolume, (1. / 3.));
		}
		if (saturationList[i] > 1.0) {
...
{
	double waterVolume = 0.0, pressureWaterVolume = 0.0, waterVolume_NHJ = 0.0, pressureWaterVolume_NHJ = 0.0, waterVolumeP = 0.0, YDimension = 0.0,
		...
		if (listOfPores[i]->info().hasInterface) {
			if (listOfPores[i]->info().saturation < 1.0 && listOfPores[i]->info().saturation >= listOfPores[i]->info().thresholdSaturation) {
				areaSphere = 4.0 * 3.14159265359
				        * std::pow(getChi(listOfPores[i]->info().numberFacets)
				                           * std::pow(listOfPores[i]->info().mergedVolume * (1.0 - listOfPores[i]->info().saturation), 0.3333),
				                   2);
			}
			if (listOfPores[i]->info().saturation < listOfPores[i]->info().thresholdSaturation && listOfPores[i]->info().saturation > 0.0
				...
				areaSphere = 4.0 * 3.14159265359 * std::pow((2.0 * surfaceTension / (-1.0 * listOfPores[i]->info().p())), 2.0)
	...
	if (!deformation) {
		double volumeWaterVBC = 0.0, volumeWaterPressureBC = 0.0, volumeWaterBC = 0.0, volumeWaterAirBC = 0.0, volumeWaterPressureAirBC = 0.0,
		...
		if (!cell->info().isFictious) {
			for (unsigned int j = 0; j < 4; j++) {
				if (cell->info().poreId != cell->neighbor(j)->info().poreId && cell->info().id > cell->neighbor(j)->info().id) {
					...
					double A = ((std::pow(rA, 2)) * (1 - c1[0] / b1[0]) + ((std::pow(rB, 2) * c1[0]) / b1[0]) - std::pow(rC, 2)
					            + pow(c1[0], 2) + std::pow(c1[1], 2) - ((std::pow(b1[0], 2) + std::pow(b1[1], 2)) * c1[0] / b1[0]))
		...
		if (!cell->info().isFictious) {
			...
			if (deformation) {
				...
				cell->info().poreBodyRadius
				        = getChi(cell->info().numberFacets) * std::pow(listOfPores[cell->info().poreId]->info().mergedVolume, (1. / 3.));
