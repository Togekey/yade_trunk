...
GLLock::GLLock(GLViewer* _glv)
        : boost::try_mutex::scoped_lock(Omega::instance().renderMutex)
        , glv(_glv)
{
	glv->makeCurrent();
}
...
{ /* get the GL mutex when closing */
	GLLock lock(this); /* cerr<<"Destructing view #"<<viewId<<endl;*/
}
...
void GLViewer::startClipPlaneManipulation(int planeNo)
{
	...
	manipulatedFrame()->setPositionAndOrientation(
	        qglviewer::Vec(se3.position[0], se3.position[1], se3.position[2]),
	        qglviewer::Quaternion(se3.orientation.x(), se3.orientation.y(), se3.orientation.z(), se3.orientation.w()));
...
void GLViewer::centerScene()
{
	...
	if (minDim <= 0 || hasNan) {
		...
		if (std::isinf(min[0]) || std::isinf(min[1]) || std::isinf(min[2]) || std::isinf(max[0]) || std::isinf(max[1]) || std::isinf(max[2])) {
...
void GLViewer::initFromDOMElement(const QDomElement& element)
{
	...
	while (!child.isNull()) {
		...
		if (child.tagName() == "timeDisplay" && child.hasAttribute("mask"))
			timeDispMask = atoi(child.attribute("mask").toAscii());
