...
O.engines=[
	...
	PartialSatClayEngine(dead=1,label="flow"),#introduced as a dead engine for the moment, see 2nd section
...
def history():
	...
	centerSat=flow.getCellSaturation((0.5,0.5,0.5)))
  	#plot.addData(e22=-triax.strain[1],t=O.time,s22=-triax.stress(2)[1],p=flow.MeasurePorePressure((0.5,0.5,0.5)))
	plot.saveDataTxt('txt'+timeStr+identifier+'/saturation'+identifier+'.txt',vars=('t','p','centerSat'))

O.engines=O.engines+[PyRunner(iterPeriod=200,command='history()',label='recorder')]
##make nice animations:
def pressureField():
	flow.saveUnsatVtk('VTK'+timeStr+identifier+'/',withBoundaries=False)
O.engines=O.engines+[PyRunner(iterPeriod=200,command='pressureField()')]

from yade import plot
plot.plots={'t':('centerSat')}
plot.plot()
O.saveTmp()
O.timingEnabled=1
from yade import timing
